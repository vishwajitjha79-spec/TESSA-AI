      ...
      >
        {callFullscreen ? (
          ...
          <>
            ...
            {/* Waveform visualiser — fullscreen version, bars grow from bottom */}
            <div style={{display:'flex', alignItems:'flex-end', justifyContent:'center', gap:3, height:72, width:'100%', padding:'0 28px', flexShrink:0}}>
              {barPattern.map((bh,i)=>(
                <div key={i} style={{
                  ...
                  background: isActive
                    ? `linear-gradient(0deg, ${sc} 0%, ${sc}88 100%)`
                  ...
                  boxShadow: isActive ? `0 0 8px ${sc}60` : 'none',
                }}/>
              ))}
            </div>

            {/* Transcript — fullscreen scrollable */}
            <div style={{
              width:'100%', maxWidth:480, padding:'0 20px', flexShrink:0,
              maxHeight:180, overflowY:'auto',
            }}>
              <div style={{
                borderRadius:18, background:'rgba(255,255,255,0.05)',
                ...
                {callTranscript.length===0 ? (
                  ...
                  [...callTranscript].slice(-6).map((m,i)=>(
                    <div key={i} style={{
                      marginBottom: i < callTranscript.slice(-6).length-1 ? 10 : 0,
                    }}>
                      <span style={{
                      ...
                      <p style={{
                        fontSize:13, fontWeight: m.role==='tessa'?600:400,
                        color:m.role==='user'?'rgba(255,255,255,0.78)':'#ffffff',
                ...
                {interimText && (
                  <p style={{
                    fontSize:12, color:sc, fontStyle:'italic',
        ...
        {!showDashboard&&(
          <div className={`flex-shrink-0 ${t.bar} px-3 md:px-6 py-3 safe-bottom`}>
            <div className="max-w-2xl mx-auto w-full">
              ...
              {/* ══ FEATURE DOCK — pills on desktop, collapsed tray on mobile ══ */}
              <div className="mt-2 flex items-center gap-1.5">

                {/* Web search pill — always visible */}
                <button onClick={()=>setWebSearchOn(p=>!p)}
                  className="flex-shrink-0 flex items-center gap-1.5 h-7 px-3 rounded-full transition-all active:scale-95"
                  style={{
                    background: webSearchOn ? `linear-gradient(135deg,${t.glow}30,${t.glow}18)` : t.isLight?'rgba(0,0,0,0.05)':'rgba(255,255,255,0.05)',
                    border: `1px solid ${webSearchOn ? t.glow+'60' : t.isLight?'rgba(0,0,0,0.1)':'rgba(255,255,255,0.1)'}`,
                  }}>
                  <Globe size={10} style={{color:webSearchOn?t.glow:undefined,opacity:webSearchOn?1:0.5}}/>
                  <span className="text-[10px] font-semibold" style={{color:webSearchOn?t.glow:t.isLight?'#64748b':'#94a3b8'}}>
                    {webSearchOn?'Web ON':'Web'}
                  </span>
                  {webSearchOn && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"/>}
                </button>

                {/* ── MOBILE: single "Tools" button that opens a tray ── */}
                <div className="sm:hidden relative flex-shrink-0">
                  <button
                    onClick={()=>setShowToolsDock(p=>!p)}
                    className="flex items-center gap-1.5 h-7 px-3 rounded-full transition-all active:scale-95"
                    style={{
                      background: showToolsDock ? `linear-gradient(135deg,${t.glow}25,${t.glow}10)` : t.isLight?'rgba(0,0,0,0.05)':'rgba(255,255,255,0.05)',
                      border:`1px solid ${showToolsDock?t.glow+'50':t.isLight?'rgba(0,0,0,0.1)':'rgba(255,255,255,0.1)'}`,
                    }}>
                    <span style={{fontSize:11}}>⚙️</span>
                    <span className="text-[10px] font-semibold" style={{color:showToolsDock?t.glow:t.isLight?'#64748b':'#94a3b8'}}>Tools</span>
                    {tessaPersona!=='companion' && (
                      <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{background:t.glow}}/>
                    )}
                  </button>
                  ...
                  {showToolsDock && (
                    <>
                      ...
                      <div className="absolute bottom-full left-0 mb-2 z-[56] rounded-2xl overflow-hidden shadow-2xl"
                        style={{
                          minWidth:220,
                          background:t.isLight?'rgba(255,253,248,0.98)':'rgba(10,12,26,0.98)',
                          border:`1.5px solid ${t.glow}30`,
                          boxShadow:`0 -8px 40px rgba(0,0,0,0.4), 0 0 0 1px ${t.glow}10`,
                          backdropFilter:'blur(24px)',
                        }}>
                        {/* Tray header */}
                        <div className="flex items-center justify-between px-3 py-2.5"
                          style={{borderBottom:`1px solid ${t.glow}14`}}>
                          <span className="text-[10px] font-black uppercase tracking-widest" style={{color:t.glow}}>Tools</span>
                          <button onClick={()=>setShowToolsDock(false)}
                            className="w-5 h-5 flex items-center justify-center rounded-lg opacity-40 hover:opacity-70">
                            <X size={11}/>
                          </button>
                        </div>
                        {/* Grid of tools */}
                        <div className="p-2 grid grid-cols-3 gap-1.5">
                          {[
                            ...
                            <button key={item.lbl}
                              ...
                              style={{
                                background: item.active ? `${t.glow}15` : t.isLight?'rgba(0,0,0,0.03)':'rgba(255,255,255,0.04)',
                                border:`1px solid ${item.active?t.glow+'35':t.glow+'0e'}`,
                              }}>
                              <span style={{fontSize:18,lineHeight:1}}>{item.ico}</span>
                              <span className="text-[9px] font-semibold text-center leading-tight"
                                style={{color:item.active?t.glow:t.isLight?'#64748b':'#8b949e'}}>
                                {item.lbl}
                              </span>
                              {(item.badge??0)>0&&(
                                <span className="absolute top-1 right-1 w-3.5 h-3.5 rounded-full flex items-center justify-center text-white font-black"
                ...
                <div className="hidden sm:flex items-center gap-1 overflow-x-auto"
                  ...
                  {[
                    ...
                    {ico:'✦',  lbl:tessaPersona!=='companion'?tessaPersona:'Persona', fn:()=>setShowProfiles(true), active:tessaPersona!=='companion'},
                    ...
                    <button key={btn.lbl} onClick={btn.fn}
                      className="relative flex-shrink-0 flex items-center gap-1.5 h-7 px-3 rounded-full transition-all active:scale-95"
                      style={{
                        background: btn.active ? `linear-gradient(135deg,${t.glow}28,${t.glow}14)` : t.isLight?'rgba(0,0,0,0.05)':'rgba(255,255,255,0.05)',
                        border:`1px solid ${btn.active?t.glow+'50':t.isLight?'rgba(0,0,0,0.09)':'rgba(255,255,255,0.09)'}`,
                      }}>
                      <span style={{fontSize:11,lineHeight:1}}>{btn.ico}</span>
                      <span className="text-[10px] font-medium" style={{color:btn.active?t.glow:t.isLight?'#64748b':'#94a3b8'}}>
                        {btn.lbl}
                      </span>
                      {(btn.badge??0)>0&&(
                        <span className="absolute -top-1 -right-0.5 w-3.5 h-3.5 rounded-full flex items-center justify-center text-white font-black"
                          style={{background:t.glow,fontSize:7}}>{btn.badge??''}</span>
                      )}
                    </button>
                  ))}
                </div>
                ...
                {/* Sources pill */}
                {lastSearchData && (
                  <button onClick={()=>setShowSearchPanel(p=>!p)} className="flex-shrink-0 flex items-center gap-1.5 h-7 px-3 rounded-full transition-all active:scale-95"
                    style={{background:showSearchPanel?`${t.glow}20`:t.isLight?'rgba(0,0,0,0.05)':'rgba(255,255,255,0.05)',border:`1px solid ${t.glow}30`}}>
                    <ExternalLink size={10} style={{color:t.glow}}/>
                    <span className="text-[10px] font-medium" style={{color:t.glow}}>Sources ({lastSearchData.sources.length})</span>
                  </button>
                )}
