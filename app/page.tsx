...
const THEMES = {
  ...
  ankit: {
    // Standard (non-creator): warm white + soft teal accent
    bg:'bg-[#f0f8f6]', bgC:'bg-[#f0f8f6]',
    panel:'bg-white/85 backdrop-blur-xl border-teal-200/40 shadow-lg shadow-teal-100/20',
    panelC:'bg-white/85 backdrop-blur-xl border-teal-200/40 shadow-lg shadow-teal-100/20',
    header:'bg-[#f0f8f6]/95 backdrop-blur-xl border-b border-teal-200/35 shadow-sm',
    headerC:'bg-[#f0f8f6]/95 backdrop-blur-xl border-b border-teal-200/35 shadow-sm',
    bar:'bg-[#f0f8f6]/95 backdrop-blur-xl border-t border-teal-200/35',
    barC:'bg-[#f0f8f6]/95 backdrop-blur-xl border-t border-teal-200/35',
    // User messages: teal left border
    msgU:'bg-white/80 border border-teal-200/40 border-l-[3px] border-l-teal-500/60 shadow-sm',
    msgUC:'bg-white/80 border border-teal-200/40 border-l-[3px] border-l-cyan-500/60 shadow-sm',
    // Tessa messages: indigo left border
    msgA:'bg-white/75 border border-indigo-100/50 border-l-[3px] border-l-indigo-400/55 shadow-sm',
    msgAC:'bg-white/75 border border-indigo-100/50 border-l-[3px] border-l-indigo-500/55 shadow-sm',
    inp:'bg-white/95 border border-teal-200/50 text-slate-800 placeholder:text-slate-400 focus:border-teal-400/60 focus:ring-2 focus:ring-teal-200/30',
    inpC:'bg-white/95 border border-teal-200/50 text-slate-800 placeholder:text-slate-400 focus:border-teal-400/60 focus:ring-2 focus:ring-teal-200/30',
    // Send button: teal gradient (user); indigo gradient (creator)
    btnP:'bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white shadow-md shadow-teal-400/30',
    btnPC:'bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white shadow-md shadow-indigo-400/30',
    btnS:'bg-white/90 hover:bg-white border border-teal-200/50 text-slate-700 hover:text-slate-900',
    btnSC:'bg-white/90 hover:bg-white border border-indigo-200/50 text-slate-700 hover:text-slate-900',
    text:'text-slate-800', sub:'text-slate-500', subC:'text-slate-600',
    accent:'text-teal-600', accentC:'text-indigo-700',
    glow:'#0d9488', glowC:'#4f46e5',
    card:'bg-white/80 border border-teal-100/50',
    cardC:'bg-white/80 border border-indigo-100/50',
    active:'bg-white border border-teal-400/50 shadow-sm',
    activeC:'bg-white border border-indigo-500/50 shadow-sm',
    div:'border-teal-200/35', divC:'border-indigo-200/35',
    settBg:'bg-white/98 backdrop-blur-xl border-t border-teal-200/40',
    settBgC:'bg-white/98 backdrop-blur-xl border-t border-indigo-200/40',
    ...
    settActive:'bg-teal-50 border border-teal-300/50',
    isLight:true,
  },
} as const;
...
function buildCreatorPayload() {
  // Collect conversations (last 40 only — keep payload < 5MB)
  const rawConvs = lsGetJson<any[]>('tessa-conversations', []);
  const conversations = rawConvs.slice(-40).map((c: any) => ({
    ...c,
    messages: Array.isArray(c.messages) ? c.messages.slice(-60) : [],
  }));
  return {
    // Health & wellness
    ...
    wellness:    lsGetJson('tessa-wellness',  null),
    calHistory:  lsGetJson('tessa-cal-history', {}),
    workoutPlan: lsGetJson('tessa-workout-plan', []),
    savedPlanners: lsGetJson('tessa-saved-planners', []),
    // Chat history
    conversations,
    // Identity & appearance
    ...
    persona:     lsGet('tessa-persona')      ?? 'companion',
    // All settings
    settings: {
      animations:     lsGet('tessa-animations')      ?? 'true',
      sfx:            lsGet('tessa-sfx')             ?? 'true',
      fontSize:       lsGet('tessa-font-size')       ?? 'base',
      compact:        lsGet('tessa-compact')         ?? 'false',
      voiceOutput:    lsGet('tessa-voice-output')    ?? 'false',
      responseLen:    lsGet('tessa-response-length') ?? 'balanced',
      sendEnter:      lsGet('tessa-send-enter')      ?? 'true',
      typingEffect:   lsGet('tessa-typing-effect')   ?? 'true',
      timestamps:     lsGet('tessa-show-timestamps') ?? 'false',
      autoMemory:     lsGet('tessa-auto-memory')     ?? 'true',
      proactive:      lsGet('tessa-proactive')       ?? 'true',
      language:       lsGet('tessa-language')        ?? 'en',
      voicePersonality: lsGet('tessa-voice-personality') ?? 'warm',
      webSearch:      lsGet('tessa-auto-search')     ?? 'false',
      shortnotes:     lsGet('tessa-shortnotes')      ?? '[]',
    },
...
async function pullCreatorSync(_userId?: string): Promise<boolean> {
  try {
    ...
    if (p.wellness)   lsSet('tessa-wellness',    JSON.stringify(p.wellness));
    if (p.calHistory)    lsSet('tessa-cal-history',     JSON.stringify(p.calHistory));
    if (p.workoutPlan)   lsSet('tessa-workout-plan',    JSON.stringify(p.workoutPlan));
    if (p.savedPlanners) lsSet('tessa-saved-planners',  JSON.stringify(p.savedPlanners));
    // Conversations — merge remote and local, deduplicate by id, keep newest
    if (Array.isArray(p.conversations) && p.conversations.length > 0) {
      const localConvs = lsGetJson<any[]>('tessa-conversations', []);
      const merged: Record<string,any> = {};
      [...localConvs, ...p.conversations].forEach(c => {
        if (!merged[c.id] || new Date(c.updated) > new Date(merged[c.id].updated)) {
          merged[c.id] = c;
        }
      });
      const sorted = Object.values(merged).sort((a,b)=>new Date(b.updated).getTime()-new Date(a.updated).getTime());
      lsSet('tessa-conversations', JSON.stringify(sorted.slice(0,80)));
    }
    // Identity & appearance
    if (p.mood)    lsSet('tessa-last-mood', String(p.mood));
    if (p.avatar)  lsSet('tessa-avatar',   String(p.avatar));
    if (p.theme)   lsSet('tessa-theme',    String(p.theme));
    if (p.persona) lsSet('tessa-persona',  String(p.persona));
    // Settings
    if (p.settings) {
      const s = p.settings;
      if (s.animations)       lsSet('tessa-animations',       s.animations);
      if (s.sfx)              lsSet('tessa-sfx',              s.sfx);
      if (s.fontSize)         lsSet('tessa-font-size',        s.fontSize);
      if (s.compact)          lsSet('tessa-compact',          s.compact);
      if (s.voiceOutput)      lsSet('tessa-voice-output',     s.voiceOutput);
      if (s.responseLen)      lsSet('tessa-response-length',  s.responseLen);
      if (s.sendEnter)        lsSet('tessa-send-enter',       s.sendEnter);
      if (s.typingEffect)     lsSet('tessa-typing-effect',    s.typingEffect);
      if (s.timestamps)       lsSet('tessa-show-timestamps',  s.timestamps);
      if (s.autoMemory)       lsSet('tessa-auto-memory',      s.autoMemory);
      if (s.proactive)        lsSet('tessa-proactive',        s.proactive);
      if (s.language)         lsSet('tessa-language',         s.language);
      if (s.voicePersonality) lsSet('tessa-voice-personality',s.voicePersonality);
      if (s.webSearch)        lsSet('tessa-auto-search',      s.webSearch);
      if (s.shortnotes)       lsSet('tessa-shortnotes',       s.shortnotes);
    }
...
function LightBg({ creator, theme }: { creator: boolean; theme: Theme }) {
  ...
  if (theme === 'ankit' && creator) {
    // Creator theme: clean soft light bg — warm ivory base + teal-mint top-left + deep indigo bottom-right
    return (
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0" aria-hidden>

        {/* Ivory cream base */}
        <div className="absolute inset-0"
          style={{ background: 'linear-gradient(150deg, #f0faf8 0%, #e8f7f4 40%, #f0f4ff 75%, #f5faf9 100%)' }}/>

        {/* Teal-mint wash — top-left */}
        <div className="absolute" style={{
          top:-100, left:-80, width:500, height:480, borderRadius:'50%',
          background:'radial-gradient(circle, rgba(13,148,136,0.10) 0%, rgba(45,212,191,0.06) 50%, transparent 72%)',
          filter:'blur(48px)',
        }}/>

        {/* Indigo-blue wash — bottom-right */}
        <div className="absolute" style={{
          bottom:-120, right:-100, width:560, height:520, borderRadius:'50%',
          background:'radial-gradient(circle, rgba(37,99,235,0.08) 0%, rgba(99,102,241,0.05) 50%, transparent 72%)',
          filter:'blur(52px)',
        }}/>

        {/* Soft centre glow — teal tint */}
        <div className="absolute inset-0" style={{
          background:'radial-gradient(ellipse 65% 50% at 50% 50%, rgba(13,148,136,0.05) 0%, transparent 70%)',
        }}/>

        {/* Fine dot grid — teal dots */}
        <div className="absolute inset-0" style={{
        ...
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 800"
          preserveAspectRatio="xMidYMid slice" style={{ opacity: 0.022 }}>
          {Array.from({length:30},(_,i)=>(
            <line key={i}
              x1={-60 + i*16} y1="0"
              x2={-60 + i*16 + 200} y2="800"
              stroke="#0f4c3a" strokeWidth={i%5===0?0.8:0.4}/>
          ))}
        </svg>

        {/* Teal corner accent — top right */}
        <div className="absolute top-0 right-0" style={{
          width:180, height:180,
          background:'radial-gradient(circle at 100% 0%, rgba(13,148,136,0.09) 0%, transparent 60%)',
        }}/>

        {/* Indigo corner accent — bottom left */}
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center">
      ...
      <div className={`relative w-full max-w-sm mx-4 mb-4 sm:mb-0 rounded-3xl overflow-hidden shadow-2xl ${t.isLight?'bg-white border border-slate-200':'bg-[#0d1120] border border-white/10'}`}
        ...
        <div className="p-4 grid grid-cols-4 gap-4" style={{isolation:'isolate'}}>
          {AVATARS.map(av => {
            ...
            return (
              <button key={av.id} onClick={() => { onSelect(av.id); onClose(); }}
                ...
                {/* Outer ring — selected indicator — lives outside the image so it never clips */}
                <div className="relative" style={{padding: isSel ? 3 : 2}}>
                  <div className="w-14 h-14 rounded-2xl overflow-hidden"
                    style={{
                      boxShadow: isSel
                        ? `0 0 0 0 transparent, 0 4px 14px ${glow}35`
                        : '0 2px 6px rgba(0,0,0,0.12)',
                      transform: 'none', // NO scale — prevents grid overlap
                    ...
                    {isSel && (
                      <div className="absolute inset-0 flex items-start justify-end p-1" style={{background:`${glow}14`}}>
                        <div className="w-4 h-4 rounded-full flex items-center justify-center" style={{background:glow,boxShadow:`0 0 6px ${glow}`}}>
                          <Check size={8} className="text-white" />
                        </div>
                      </div>
                    )}
                  </div>
                  {/* Selected ring drawn as outline — outside the img so no overflow clip needed */}
                  {isSel && (
                ...
                <p className="text-[8px] font-semibold truncate w-full text-center"
                  style={{color: isSel ? glow : undefined, opacity: isSel ? 1 : 0.55}}>
                  {av.emoji} {av.name}
                </p>
              </button>
            );
...
STRICT: Be short. No fluff. Non-health topics: "Use main Tessa chat for that!"`;
    ...
    try {
      ...
      if (calMatch) {
        ...
        try {
          ...
          // Also update 7-day calorie history for dashboard chart
          try {
            const hist = JSON.parse(localStorage.getItem('tessa-cal-history') || '{}');
            hist[today] = h.totalCalories;
            // Keep only last 7 days
            const keys = Object.keys(hist).sort();
            if (keys.length > 7) delete hist[keys[0]];
            localStorage.setItem('tessa-cal-history', JSON.stringify(hist));
          } catch {}
          h.meals = [...(h.meals || []), {
          ...
          onSync?.();
          // Universal cross-device calorie sync (creator mode)
          try { if (typeof isCreatorModePersistent === 'function' && isCreatorModePersistent()) pushCreatorSync(); } catch {}
        } catch {}
      }
  ...
  // FAB drag — direct DOM transform, no React state during drag
  const onFabPtrDown = (e: React.PointerEvent<HTMLButtonElement>) => {
    if (inDock || open) return;
    fabDragging.current = false; // reset
    fabMoved.current = false;
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
    fabDragStart.current = {
      mx: e.clientX, my: e.clientY,
      px: fabPos?.x ?? rect.left,
      py: fabPos?.y ?? rect.top,
    };
    (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId);
  };
  const onFabPtrMove = (e: React.PointerEvent<HTMLButtonElement>) => {
    if (inDock || open) return;
    const dx = e.clientX - fabDragStart.current.mx;
    const dy = e.clientY - fabDragStart.current.my;
    if (!fabDragging.current && Math.sqrt(dx*dx+dy*dy) < 6) return; // deadzone
    fabDragging.current = true;
    fabMoved.current = true;
    const nx = Math.max(8, Math.min(window.innerWidth-56,  fabDragStart.current.px + dx));
    const ny = Math.max(8, Math.min(window.innerHeight-56, fabDragStart.current.py + dy));
    if (fabRef.current) {
      fabRef.current.style.left   = nx+'px';
      fabRef.current.style.top    = ny+'px';
      fabRef.current.style.bottom = 'unset';
      fabRef.current.style.right  = 'unset';
    }
  };
  const onFabPtrUp = (e: React.PointerEvent<HTMLButtonElement>) => {
    if (inDock || open) return;
    if (fabDragging.current) {
      const nx = Math.max(8, Math.min(window.innerWidth-56,  fabDragStart.current.px + e.clientX - fabDragStart.current.mx));
      const ny = Math.max(8, Math.min(window.innerHeight-56, fabDragStart.current.py + e.clientY - fabDragStart.current.my));
      setFabPos({x:nx, y:ny});
      fabDragging.current = false;
    } else {
      // Pure tap — toggle panel
      setOpen(p => !p);
    }
  };

  const fabStyle: React.CSSProperties = inDock ? { position:'relative' } : fabPos
    ? { position:'fixed', left:fabPos.x, top:fabPos.y, bottom:'unset', right:'unset' }
    : { position:'fixed', bottom:88, right:16 };

  return (
    <>
      {/* ── FAB — draggable when closed, dock-aware ── */}
      <button
        ref={fabRef}
        onClick={inDock||open ? ()=>setOpen(p=>!p) : undefined}
        onPointerDown={onFabPtrDown}
        onPointerMove={onFabPtrMove}
        onPointerUp={onFabPtrUp}
        onPointerCancel={onFabPtrUp}
        ...
        style={{
          ...
          cursor: inDock || open ? 'pointer' : 'grab',
          touchAction: 'none',
          userSelect: 'none',
          zIndex: inDock ? undefined : 60,
          willChange: 'transform',
          transition: fabDragging.current ? 'none' : 'background 0.2s',
        }}
        title={open ? 'Close' : 'Health Pulse — drag to move'}
      >
      ...
      {open && (
        <div
          ...
          onTouchStart={e=>{
            if(e.touches.length===2){
              ...
              pinchRef.current={dist,w:panelW,h:panelH};
              e.preventDefault();
            }
          }}
          onTouchMove={e=>{
            if(e.touches.length===2&&pinchRef.current){
              ...
              const nw=Math.max(280,Math.min(window.innerWidth-32,Math.round(pinchRef.current.w*scale)));
              const nh=Math.max(320,Math.min(window.innerHeight-60,Math.round(pinchRef.current.h*scale)));
        ...
        >
          ...
          <div className="flex-shrink-0 flex justify-center items-center gap-2 pt-2.5 pb-1.5 cursor-grab active:cursor-grabbing select-none"
            style={{ borderBottom: `1px solid ${glow}12`, position:'relative' }}>
            ...
            <div className="w-8 h-1 rounded-full" style={{ background: `${glow}35` }}/>
            {/* SE resize corner — pointer + touch */}
            <div
              style={{position:'absolute',right:0,top:0,bottom:0,width:32,cursor:'se-resize',
                display:'flex',alignItems:'center',justifyContent:'center',touchAction:'none',zIndex:10}}
              onPointerDown={e=>{
                e.stopPropagation();
                resizing.current=true;
                resizeStart.current={mx:e.clientX,my:e.clientY,w:panelW,h:panelH};
                (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId);
              }}
              onPointerMove={e=>{
                if(!resizing.current)return;
                const nw=Math.max(280,Math.min(window.innerWidth-32,resizeStart.current.w+(e.clientX-resizeStart.current.mx)));
                const nh=Math.max(320,Math.min(window.innerHeight-60,resizeStart.current.h+(e.clientY-resizeStart.current.my)));
                setPanelW(nw); setPanelH(nh);
              }}
            ...
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M2 12L12 2M6 12L12 6M12 12V12" stroke={glow} strokeWidth="1.8" strokeLinecap="round"/>
              </svg>
            </div>
          </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// STUDY HEATMAP — GitHub contribution-style
// ─────────────────────────────────────────────────────────────────────────────
function StudyHeatmap({ log, glow, isLight }: { log: Record<string,number>; glow: string; isLight: boolean }) {
  ...
  const getColor = (count: number) => {
    if (count === 0) return isLight ? '#e2e8f0' : '#1c2333';
    const intensity = count / maxCount;
    return intensity > 0.75 ? glow
         : intensity > 0.5  ? glow + 'bb'
         : intensity > 0.25 ? glow + '77'
         :                    glow + '44';
  };
    ...
    <div className="overflow-x-auto pb-1" style={{scrollbarWidth:'thin'}}>
      <div className="flex gap-0.5">
        {weeks.map((week, wi) => (
          <div key={wi} className="flex flex-col gap-0.5">
            {week.map((day, di) => (
              <div key={di} title={`${day.date}: ${day.count} messages`}
                className="rounded-sm transition-transform hover:scale-125"
                style={{ width:10, height:10, background: getColor(day.count), flexShrink:0 }}/>
            ))}
          </div>
        ))}
      </div>
      <div className="flex items-center justify-between mt-1.5">
        <div className="flex items-center gap-1">
          <span className="text-[8px]" style={{color:isLight?'#94a3b8':'#475569'}}>Less</span>
          {[0,0.25,0.5,0.75,1].map((v,i)=>(
            <div key={i} className="rounded-sm" style={{width:8,height:8,background:v===0?(isLight?'#e2e8f0':'#1c2333'):glow+(v===1?'':v===0.75?'bb':v===0.5?'77':'44')}}/>
          ))}
          <span className="text-[8px]" style={{color:isLight?'#94a3b8':'#475569'}}>More</span>
        </div>
        <span className="text-[8px]" style={{color:isLight?'#94a3b8':'#475569'}}>
          {Object.values(log).reduce((a,b)=>a+b,0)} total messages this year
        </span>
      </div>
    </div>
  );
...
// TIME CAPSULE PANEL
// ─────────────────────────────────────────────────────────────────────────────
function TimeCapsulePanel({ glow, isLight, capsules, onSave, onClose }: {
  glow:string; isLight:boolean;
  capsules:{id:string;msg:string;deliverOn:string;note:string;opened:boolean}[];
  onSave:(c:{id:string;msg:string;deliverOn:string;note:string;opened:boolean})=>void;
  onClose:()=>void;
}) {
  const [msg, setMsg] = useState('');
  const [date, setDate] = useState('');
  const inpStyle = { background:isLight?'#f8fafc':'#1c2333', border:`1px solid ${glow}25`, color:isLight?'#1e293b':'#e2e8f0' };
  const minDate = new Date(); minDate.setDate(minDate.getDate()+1);
  const minStr = minDate.toISOString().split('T')[0];
  return (
    <div className="w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl max-h-[85vh] flex flex-col"
      style={{background:isLight?'#fff':'#0c0f1a',border:`1px solid ${glow}30`}}>
      <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0" style={{borderColor:`${glow}18`}}>
        <div className="flex items-center gap-2"><span style={{fontSize:18}}>📬</span>
          <span className="font-black text-[14px]" style={{color:glow}}>Time Capsule</span></div>
        <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center text-slate-400 hover:text-white transition-colors"><X size={14}/></button>
      </div>
      <div className="px-5 py-4 space-y-3 overflow-y-auto" style={{scrollbarWidth:'thin'}}>
        <p className={`text-[11px] leading-relaxed ${isLight?'text-slate-600':'text-white/60'}`}>
          Write a message to your future self. Tessa will deliver it on your chosen date.
        </p>
        <div>
          <label className="text-[10px] font-bold block mb-1" style={{color:glow}}>Your message</label>
        ...
        <div>
          <label className="text-[10px] font-bold block mb-1" style={{color:glow}}>Deliver on</label>
          <input type="date" value={date} min={minStr} onChange={e=>setDate(e.target.value)}
            className="w-full px-3 py-2 rounded-xl text-[12px] outline-none" style={inpStyle}/>
        </div>
        <button onClick={()=>{
          if (!msg.trim()||!date){alert('Please write a message and pick a date!');return;}
          const tessaNote = `Sealed on ${new Date().toLocaleDateString('en-IN',{day:'numeric',month:'long',year:'numeric'})}. Tessa whispers: *I kept this safe for you.* 💝`;
          onSave({id:Math.random().toString(36).slice(2),msg:msg.trim(),deliverOn:date,note:tessaNote,opened:false});
        }} className="w-full py-3 rounded-2xl font-black text-[13px] text-white"
          style={{background:`linear-gradient(135deg,${glow},${glow}bb)`,boxShadow:`0 4px 18px ${glow}40`}}>
          📬 Seal & Send to Future
        </button>
        {capsules.length > 0 && (
          <div>
            <p className="text-[10px] font-black mb-2" style={{color:glow}}>PENDING CAPSULES ({capsules.filter(c=>!c.opened).length})</p>
            {capsules.filter(c=>!c.opened).map((cap,i)=>(
              <div key={i} className="px-3 py-2 rounded-xl mb-1.5" style={{background:isLight?'#f8fafc':'#1c2333',border:`1px solid ${glow}18`}}>
                <p className={`text-[10px] font-medium ${isLight?'text-slate-700':'text-white/80'}`}>"{cap.msg.slice(0,50)}{cap.msg.length>50?'…':''}"</p>
                <p className={`text-[9px] mt-0.5 ${isLight?'text-slate-400':'text-white/40'}`}>Opens: {cap.deliverOn}</p>
              </div>
            ))}
          </div>
        )}
...
// CODE SANDBOX PANEL
// ─────────────────────────────────────────────────────────────────────────────
function CodeSandboxPanel({ glow, isLight, code, setCode, output, setOutput, onClose, onSendToTessa }: {
  glow:string; isLight:boolean; code:string; setCode:(s:string)=>void;
  output:string; setOutput:(s:string)=>void; onClose:()=>void; onSendToTessa:(code:string)=>void;
}) {
  const [running, setRunning] = useState(false);
  const codeStyle = { background:isLight?'#0f172a':'#0a0a12', color:'#e2e8f0', border:`1px solid ${glow}25`, fontFamily:'monospace' };

  const runCode = async () => {
    setRunning(true);
    setOutput('');
    try {
      // Use sandboxed iframe eval for JavaScript
      const logs: string[] = [];
  ...
  return (
    <div className="w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col"
      style={{background:isLight?'#fff':'#0c0f1a',border:`1px solid ${glow}30`,maxHeight:'85vh'}}>
      <div className="flex items-center justify-between px-5 py-3 border-b flex-shrink-0" style={{borderColor:`${glow}18`}}>
        <div className="flex items-center gap-2">
          <span style={{fontSize:16}}>⚡</span>
          <span className="font-black text-[14px]" style={{color:glow}}>Code Sandbox</span>
          <span className="text-[9px] px-2 py-0.5 rounded-full font-bold" style={{background:`${glow}15`,color:glow}}>JS</span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={()=>onSendToTessa(code)} className="text-[9px] px-2.5 py-1 rounded-lg font-semibold"
            style={{background:`${glow}15`,color:glow,border:`1px solid ${glow}30`}}>Ask Tessa</button>
          <button onClick={onClose} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 hover:text-white"><X size={12}/></button>
        </div>
      </div>
      <div className="flex-1 flex flex-col overflow-hidden" style={{minHeight:0}}>
        <textarea value={code} onChange={e=>setCode(e.target.value)} rows={10}
          placeholder="// Write JavaScript here…&#10;console.log('Hello Tessa!');"
          className="flex-1 px-4 py-3 text-[12px] outline-none resize-none"
          style={codeStyle}/>
        <div className="px-3 py-2 border-t flex items-center gap-2" style={{borderColor:`${glow}15`}}>
          <button onClick={runCode} disabled={running||!code.trim()}
            className="px-4 py-2 rounded-xl text-[11px] font-black text-white disabled:opacity-40 transition-all active:scale-95"
            style={{background:`linear-gradient(135deg,${glow},${glow}99)`,boxShadow:`0 2px 12px ${glow}35`}}>
            {running?'Running…':'▶ Run'}
          </button>
          <button onClick={()=>{setCode('');setOutput('');}} className="text-[10px] px-3 py-2 rounded-xl"
            style={{border:`1px solid ${glow}20`,color:glow}}>Clear</button>
        </div>
        {(output||running) && (
          <div className="px-4 py-3 border-t overflow-auto" style={{borderColor:`${glow}15`,maxHeight:140,background:isLight?'#f8fafc':'#080c14'}}>
            <p className="text-[9px] font-black mb-1" style={{color:glow}}>OUTPUT</p>
            <pre className="text-[11px] font-mono whitespace-pre-wrap" style={{color:output.startsWith('❌')?'#ef4444':'#4ade80'}}>
...
// SELF-REPORT PANEL
// ─────────────────────────────────────────────────────────────────────────────
function SelfReportPanel({ glow, isLight, report, setReport, messages, isLoading, onClose }: {
  glow:string; isLight:boolean; report:string; setReport:(s:string)=>void;
  messages:any[]; isLoading:boolean; onClose:()=>void;
}) {
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      const summary = messages.slice(-20).map(m=>`${m.role==='user'?'User':'Tessa'}: ${(m.content||'').slice(0,80)}`).join('\n');
      const res = await fetch('/api/chat', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({
          messages:[{role:'user', content:
            `You are Tessa doing a brief honest self-evaluation of your recent performance in this conversation.\n\nRecent messages:\n${summary}\n\nWrite a short self-report (4-6 sentences) covering:\n1. What topics you helped with\n2. Where you felt confident\n3. Where you were uncertain or could have done better\n4. One thing you'd improve\n\nBe honest and self-aware. No bullet points, write naturally.`
          }],
          isCreatorMode:false, maxTokens:250, language:'en',
        }),
      });
      const d = await res.json();
      setReport(d.content || 'Could not generate report.');
    } catch { setReport('Error generating self-report.'); }
    setGenerating(false);
  };
  ...
  return (
    <div className="w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl max-h-[85vh] flex flex-col"
      style={{background:isLight?'#fff':'#0c0f1a',border:`1px solid ${glow}30`}}>
      <div className="flex items-center justify-between px-5 py-4 border-b" style={{borderColor:`${glow}18`}}>
        <div className="flex items-center gap-2"><span style={{fontSize:18}}>🪞</span>
          <span className="font-black text-[14px]" style={{color:glow}}>Tessa's Self-Report</span></div>
        <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center text-slate-400 hover:text-white"><X size={14}/></button>
      </div>
      <div className="px-5 py-4 space-y-3">
        {!report && !generating && (
          <p className={`text-[11px] leading-relaxed ${isLight?'text-slate-600':'text-white/60'}`}>
            Ask Tessa to honestly evaluate her recent performance in this conversation.
          </p>
        )}
        {generating && (
          <div className="flex items-center gap-3 py-4">
            <div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{borderColor:glow,borderTopColor:'transparent'}}/>
            <p className="text-[12px]" style={{color:glow}}>Tessa is reflecting…</p>
          </div>
        )}
        {report && (
          <div className="p-3 rounded-2xl text-[12px] leading-relaxed"
            style={{background:isLight?'#f8fafc':'#131825',border:`1px solid ${glow}15`,color:isLight?'#374151':'#e2e8f0'}}>
            {report}
          </div>
        )}
        <button onClick={generate} disabled={generating||isLoading}
          className="w-full py-3 rounded-2xl font-black text-[13px] text-white disabled:opacity-40 transition-all active:scale-98"
          style={{background:`linear-gradient(135deg,${glow},${glow}99)`,boxShadow:`0 4px 18px ${glow}35`}}>
...
// ─────────────────────────────────────────────────────────────────────────────
// CHANGELOG PANEL — v8.0 update history
// ─────────────────────────────────────────────────────────────────────────────
const CHANGELOG = [
  {
    version: 'v8.0', date: 'Mar 2026', tag: 'Current', tagColor: '#22c55e',
    updates: [
      '⚡ Predictive ghost-text completion (Tab to accept)',
      '🧠 Ambient Intelligence — typing cadence detection (Focused/Stressed/Drifting)',
      '📊 Tessa Confidence Score on every reply',
      '📐 Formula Sheet — auto-collects formulas from conversations',
      '🏆 Achievements + Study Heatmap (GitHub-style activity graph)',
      '📬 Time Capsule — write to your future self',
      '⚡ Code Sandbox — run JavaScript inline',
      '🪞 Tessa Self-Report — honest AI self-evaluation',
      '📰 Live News Ticker — BBC headlines, click to ask Tessa',
      '🎙️ Voice Call fullscreen mode + varying greetings by time of day',
      '🌤️ Weather badge (live from wttr.in)',
      '💬 Message Reactions (8 emojis, hover to reveal)',
      '✦ Smart Follow-Up chips after every reply',
    ],
  },
  {
    version: 'v7.0', date: 'Feb 2026', tag: 'Previous', tagColor: '#6366f1',
    updates: [
      '🧠 Quiz Generator with subject/class/level/timer',
      '🧬 Memory Timeline visual panel',
      '✦ Tessa Personas (Companion/Mentor/Study Buddy/Therapist/Debater)',
      '🎙️ Voice Personality (Warm/Professional/Playful/Calm)',
      '👤 Multi-profile switcher',
      '🔄 Fixed conversation save bug (localStorage quota overflow)',
      '🕐 Live date & time in header (updates every second)',
      '🌐 Real-time awareness — Tessa knows today\'s date always',
      '🗑️ Removed NOVA browser (simplified to clean Web toggle)',
    ],
  },
  {
    version: 'v6.0', date: 'Jan 2026', tag: 'Legacy', tagColor: '#94a3b8',
    updates: [
      '🌿 Ankit theme (mint-teal + indigo palette)',
      '🔍 Web search toggle + Sources panel',
      '🌍 Hinglish language mode',
      '📊 Tessa Insights panel',
      '🏃 Health Pulse — calorie & meal tracking',
      '🎙️ Voice Call (continuous phone-call loop)',
      '💾 Cloud sync via Supabase',
    ],
  },
];

function ChangelogPanel({ glow, isLight, onClose }: { glow:string; isLight:boolean; onClose:()=>void }) {
  return (
    <div className="w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl max-h-[88vh] flex flex-col"
      style={{background:isLight?'#fff':'#0c0f1a',border:`1px solid ${glow}30`}}>
      <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0" style={{borderColor:`${glow}18`}}>
        <div className="flex items-center gap-2">
          <span style={{fontSize:18}}>🚀</span>
          <span className="font-black text-[14px]" style={{color:glow}}>What's New</span>
        </div>
        <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center text-slate-400 hover:text-white"><X size={14}/></button>
      </div>
        ...
        {CHANGELOG.map((release,ri) => (
          <div key={ri}>
            <div className="flex items-center gap-2 mb-2">
              <span className="font-black text-[13px]" style={{color:glow}}>{release.version}</span>
              <span className="text-[9px] px-2 py-0.5 rounded-full font-bold"
                style={{background:`${release.tagColor}18`,color:release.tagColor,border:`1px solid ${release.tagColor}30`}}>
                {release.tag}
              </span>
              <span className={`text-[9px] ${isLight?'text-slate-400':'text-white/30'}`}>{release.date}</span>
            </div>
            <div className="space-y-1 pl-0.5">
              {release.updates.map((u,i)=>(
                <p key={i} className="text-[11px] leading-relaxed"
                  style={{color:ri===0?(isLight?'#334155':'#cbd5e1'):(isLight?'#64748b':'#64748b')}}>
                  {u}
                </p>
              ))}
            </div>
            {ri < CHANGELOG.length-1 && (
              <div className="mt-3 border-b" style={{borderColor:`${glow}12`}}/>
            )}
          </div>
        ))}
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// WORLD CLOCK + WEATHER PANEL
// ─────────────────────────────────────────────────────────────────────────────
const WORLD_CITIES = [
  { name:'New Delhi',     tz:'Asia/Kolkata',       flag:'🇮🇳', lat:28.6, lon:77.2 },
  { name:'New York',      tz:'America/New_York',   flag:'🇺🇸', lat:40.7, lon:-74.0 },
  { name:'London',        tz:'Europe/London',      flag:'🇬🇧', lat:51.5, lon:-0.1 },
  { name:'Tokyo',         tz:'Asia/Tokyo',         flag:'🇯🇵', lat:35.7, lon:139.7 },
  { name:'Dubai',         tz:'Asia/Dubai',         flag:'🇦🇪', lat:25.2, lon:55.3 },
  { name:'Sydney',        tz:'Australia/Sydney',   flag:'🇦🇺', lat:-33.9, lon:151.2 },
  { name:'Paris',         tz:'Europe/Paris',       flag:'🇫🇷', lat:48.9, lon:2.3 },
  { name:'Los Angeles',   tz:'America/Los_Angeles',flag:'🇺🇸', lat:34.1, lon:-118.2 },
  { name:'Moscow',        tz:'Europe/Moscow',      flag:'🇷🇺', lat:55.8, lon:37.6 },
  { name:'Singapore',     tz:'Asia/Singapore',     flag:'🇸🇬', lat:1.3, lon:103.8 },
  { name:'São Paulo',     tz:'America/Sao_Paulo',  flag:'🇧🇷', lat:-23.5, lon:-46.6 },
  { name:'Toronto',       tz:'America/Toronto',    flag:'🇨🇦', lat:43.7, lon:-79.4 },
];

function WorldClockPanel({ glow, isLight, onClose }: { glow:string; isLight:boolean; onClose:()=>void }) {
  const [now, setNow] = useState(new Date());
  const [selected, setSelected] = useState<typeof WORLD_CITIES[0]|null>(null);
  ...
  const fetchCityWeather = async (city: typeof WORLD_CITIES[0]) => {
    setSelected(city); setWeather(null); setWLoading(true);
    try {
      const r = await fetch(`https://wttr.in/${encodeURIComponent(city.name)}?format=j1`);
      const d = await r.json();
      const cur = d.current_condition?.[0];
      if (cur) {
        const wm: Record<string,string> = {
          '113':'☀️','116':'⛅','119':'☁️','122':'☁️','143':'🌫️','176':'🌦️',
          '200':'⛈️','227':'❄️','230':'🌨️','260':'🌫️','263':'🌧️','266':'🌧️',
          '281':'🌧️','284':'🌧️','293':'🌦️','296':'🌦️','299':'🌧️','302':'🌧️',
          '305':'🌧️','308':'🌧️','311':'🌧️','314':'🌧️','317':'🌨️','320':'❄️',
          '323':'❄️','326':'❄️','329':'❄️','332':'❄️','335':'❄️','338':'❄️',
          '350':'🧊','353':'🌦️','356':'🌧️','359':'🌧️','362':'🌨️','365':'🌨️',
          '368':'❄️','371':'❄️','374':'🧊','377':'🧊','386':'⛈️','389':'⛈️',
          '392':'⛈️','395':'❄️',
        };
        setWeather({
          temp: `${cur.temp_C}°C`,
          desc: cur.weatherDesc?.[0]?.value || '',
          icon: wm[cur.weatherCode] || '🌡️',
        });
      }
  ...
  return (
    <div className="w-full max-w-md rounded-3xl overflow-hidden shadow-2xl flex flex-col"
      style={{
        maxHeight:'88vh',
        background:isLight?'linear-gradient(160deg,#fffdf9,#faf8f4)':'linear-gradient(160deg,#0d1020,#090c17)',
        border:`1.5px solid ${isLight?'rgba(0,0,0,0.08)':glow+'30'}`,
        boxShadow:`0 32px 80px rgba(0,0,0,0.45), inset 0 1px 0 ${isLight?'rgba(255,255,255,0.9)':'rgba(255,255,255,0.06)'}`,
      }}>
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 flex-shrink-0"
        style={{borderBottom:`1px solid ${isLight?'rgba(0,0,0,0.07)':glow+'12'}`}}>
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-2xl flex items-center justify-center"
            style={{background:`linear-gradient(135deg,${glow}22,${glow}0e)`,border:`1.5px solid ${glow}40`}}>
            <span style={{fontSize:18}}>🌍</span>
          </div>
          <div>
            <p className="font-black text-[14px]" style={{color:glow}}>World Time</p>
            <p className="text-[9px]" style={{color:isLight?'#92806a':'rgba(255,255,255,0.35)'}}>
              Click any city for weather
            </p>
          </div>
        </div>
        <button onClick={onClose}
          className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-white/10 transition-colors"
          style={{border:`1px solid ${glow}20`}}>
          <X size={13} style={{color:glow}}/>
        </button>
      </div>
      ...
      <div className="px-4 pt-3 flex-shrink-0">
        <input value={search} onChange={e=>setSearch(e.target.value)}
          ...
          style={{
            ...
            border:`1px solid ${isLight?'rgba(0,0,0,0.1)':glow+'18'}`,
      ...
      {selected && (
        <div className="mx-4 mt-3 p-3.5 rounded-2xl flex-shrink-0"
          style={{
            background:isLight?'rgba(255,253,235,0.9)':'rgba(255,255,255,0.04)',
            border:`1.5px solid ${glow}25`,
          }}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] font-black" style={{color:glow}}>{selected.flag} {selected.name}</p>
              <p className="text-[20px] font-black tabular-nums mt-0.5" style={{color:isLight?'#1e293b':'#f1f5f9'}}>
                {getTime(selected.tz)}
              </p>
              <p className="text-[10px] mt-0.5" style={{color:isLight?'#78716c':'rgba(255,255,255,0.4)'}}>
                {getDate(selected.tz)}
              </p>
            </div>
            <div className="text-right">
              {wLoading && <div className="w-6 h-6 border-2 border-t-transparent rounded-full animate-spin mx-auto" style={{borderColor:glow,borderTopColor:'transparent'}}/>}
              {weather && !wLoading && (
                <>
                  <p style={{fontSize:32,lineHeight:1}}>{weather.icon}</p>
                  <p className="text-[13px] font-black mt-1" style={{color:glow}}>{weather.temp}</p>
                  <p className="text-[9px] max-w-[80px] text-right leading-tight" style={{color:isLight?'#78716c':'rgba(255,255,255,0.4)'}}>{weather.desc}</p>
                </>
              )}
            </div>
          </div>
          ...
          {filtered.map(city => {
            ...
            return (
              <button key={city.name}
                ...
                style={{
                  background: isSelected ? `${glow}12` : isLight?'rgba(0,0,0,0.03)':'rgba(255,255,255,0.03)',
                  border:`1px solid ${isSelected?glow+'40':isLight?'rgba(0,0,0,0.07)':glow+'10'}`,
                  boxShadow: isSelected ? `0 0 12px ${glow}18` : 'none',
                }}>
                <div className="flex items-center gap-1.5 mb-1">
                  <span style={{fontSize:14}}>{city.flag}</span>
                  <p className="text-[10px] font-bold truncate" style={{color:isSelected?glow:isLight?'#374151':'#e2e8f0'}}>{city.name}</p>
                </div>
                <p className="text-[13px] font-black tabular-nums" style={{color:isLight?'#1e293b':'#f1f5f9'}}>
                  {getTime(city.tz)}
                </p>
                <p className="text-[8px] mt-0.5" style={{color:isLight?'#94a3b8':'rgba(255,255,255,0.3)'}}>
                  {getDate(city.tz)}
                </p>
              </button>
            );
...
// ─────────────────────────────────────────────────────────────────────────────
function QuizGeneratorPanel({ glow, isLight, onGenerate }: {
  glow: string; isLight: boolean; onGenerate: (prompt: string) => void;
}) {
  ...
  const [qType,     setQType]     = useState<'mcq'|'short'|'long'|'mixed'>('mcq');

  const inputCls = `w-full px-3 py-2 rounded-xl text-[12px] outline-none transition-all`;
  const inputStyle = {
    background: isLight ? '#f8fafc' : '#1c2333',
    border: `1px solid ${glow}25`,
    color: isLight ? '#1e293b' : '#e2e8f0',
  };
  const labelCls = `text-[10px] font-bold mb-1 block`;

  const handleGenerate = () => {
    if (!subject.trim()) { alert('Please enter a subject!'); return; }
    const examStr   = exam ? ` for ${exam}` : '';
    const timedStr  = timed ? ` (${minutes}-minute timed quiz)` : '';
    const chapterStr = chapter ? `, chapter: ${chapter}` : '';
    const prompt = `Generate a ${qCount}-question ${qType.toUpperCase()} quiz on ${subject}${chapterStr}, Class ${cls}, difficulty: ${level}${examStr}${timedStr}. Format each question clearly numbered. For MCQ include 4 options (A-D) and mark the correct answer at the end.`;
    onGenerate(prompt);
  };
  ...
  const chip = (label: string, active: boolean, onClick: ()=>void) => (
    <button key={label} onClick={onClick}
      ...
      style={{
        background: active ? glow : isLight ? '#f1f5f9' : '#1c2333',
        color: active ? '#fff' : isLight ? '#64748b' : '#8b949e',
        border: `1px solid ${active ? glow : glow+'18'}`,
      }}>{label}</button>
  );

  return (
    <div className="px-5 py-4 space-y-3 max-h-[70vh] overflow-y-auto" style={{scrollbarWidth:'thin'}}>
      {/* Subject + Chapter */}
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className={labelCls} style={{color:glow}}>Subject *</label>
          <input className={inputCls} style={inputStyle} placeholder="e.g. Physics"
            value={subject} onChange={e=>setSubject(e.target.value)}/>
        </div>
        <div>
          <label className={labelCls} style={{color:glow}}>Chapter</label>
          <input className={inputCls} style={inputStyle} placeholder="e.g. Thermodynamics"
            value={chapter} onChange={e=>setChapter(e.target.value)}/>
        </div>
      </div>
      {/* Class */}
      <div>
        <label className={labelCls} style={{color:glow}}>Class</label>
        <div className="flex flex-wrap gap-1.5">
          {['9','10','11','12','JEE','NEET','Other'].map(v=>chip(v, cls===v, ()=>setCls(v)))}
        </div>
      </div>
      {/* Exam (optional) */}
      <div>
        <label className={labelCls} style={{color:glow}}>Target Exam <span className="font-normal opacity-60">(optional)</span></label>
        <div className="flex flex-wrap gap-1.5">
          {['','JEE Main','JEE Adv','NEET','CBSE Board','CUET','UPSC'].map(v=>chip(v||'None', exam===v, ()=>setExam(v)))}
        </div>
      </div>
      {/* Difficulty */}
      <div>
        <label className={labelCls} style={{color:glow}}>Difficulty</label>
        <div className="flex gap-1.5">
          {(['easy','medium','hard','mixed'] as const).map(v=>chip(v.charAt(0).toUpperCase()+v.slice(1), level===v, ()=>setLevel(v)))}
        </div>
      </div>
      {/* Question type */}
      <div>
        <label className={labelCls} style={{color:glow}}>Question Type</label>
        <div className="flex gap-1.5 flex-wrap">
          {(['mcq','short','long','mixed'] as const).map(v=>chip(v.toUpperCase(), qType===v, ()=>setQType(v)))}
        </div>
      </div>
      {/* Count + Timer */}
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className={labelCls} style={{color:glow}}>No. of Questions</label>
          <div className="flex items-center gap-2">
            <input type="range" min={5} max={30} value={qCount} onChange={e=>setQCount(+e.target.value)}
              className="flex-1 accent-current" style={{accentColor:glow}}/>
            <span className="text-[12px] font-bold w-6 text-center" style={{color:glow}}>{qCount}</span>
          </div>
        </div>
        <div>
          <label className={labelCls} style={{color:glow}}>Timed Quiz</label>
          <div className="flex items-center gap-2 mt-1">
            <button onClick={()=>setTimed(p=>!p)}
              className="w-9 h-5 rounded-full transition-all relative flex-shrink-0"
              style={{background:timed?glow:isLight?'#e2e8f0':'#374151'}}>
              <div className="absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all"
                style={{left:timed?'calc(100% - 18px)':'2px'}}/>
            </button>
            {timed && (
              <input type="number" min={1} max={120} value={minutes}
                onChange={e=>setMinutes(+e.target.value)}
                className={`w-14 px-2 py-1 rounded-lg text-[11px] text-center ${inputCls}`}
                style={{...inputStyle, width:56}} />
            )}
            {timed && <span className="text-[10px]" style={{color:glow}}>min</span>}
          </div>
        </div>
      </div>
      {/* Generate button */}
      <button onClick={handleGenerate}
        className="w-full py-3 rounded-2xl font-black text-[13px] text-white transition-all active:scale-98 mt-1"
        style={{background:`linear-gradient(135deg,${glow},${glow}bb)`,boxShadow:`0 4px 18px ${glow}40`}}>
        🧠 Generate Quiz
      </button>
    </div>
...
export default function Home() {
  ...
  const [liveNow,         setLiveNow]        = useState(new Date()); // live clock

  // ── New features state ────────────────────────────────────────────────────
  // Message reactions
  const [msgReactions,    setMsgReactions]   = useState<Record<string, string>>({});
  const [showReactPicker, setShowReactPicker]= useState<string|null>(null); // msgId
  // Voice personality
  const [voicePersonality,setVoicePersonality]= useState<'warm'|'professional'|'playful'|'calm'>('warm');
  // Tessa Personas
  const [tessaPersona,    setTessaPersona]   = useState<'companion'|'mentor'|'studybuddy'|'therapist'|'debater'>('companion');
  // Smart follow-ups
  const [followUps,       setFollowUps]      = useState<string[]>([]);
  const [followUpsHidden, setFollowUpsHidden]= useState(false); // user dismissed, show recall pill
  // Memory timeline panel
  const [showMemTimeline, setShowMemTimeline]= useState(false);
  // Quiz generator
  ...
  // Multi-profile
  const [profiles,        setProfiles]       = useState<{id:string;name:string;avatar:string;mode:'standard'|'creator';theme:string}[]>([]);
  const [showProfiles,    setShowProfiles]   = useState(false);
  // Voice call fullscreen
  const [callFullscreen,  setCallFullscreen] = useState(false);

  // ── v8.0 NEW FEATURES ─────────────────────────────────────────────────────
  // Predictive completion (ghost text)
  ...
  const [studyLog,        setStudyLog]       = useState<Record<string,number>>(
    () => { try { return JSON.parse(localStorage.getItem('tessa-studylog')||'{}'); } catch { return {}; } }
  );
  // Confidence score shown on replies
  const [showConfidence,  setShowConfidence] = useState(true);
  const [msgConfidence,   setMsgConfidence]  = useState<Record<string,number>>({});
  // Concept map
  const [showConceptMap,  setShowConceptMap] = useState(false);
  const [conceptMapData,  setConceptMapData] = useState<{nodes:{id:string;label:string}[];links:{source:string;target:string}[]}|null>(null);
  // Code sandbox
  ...
  const [capsules,        setCapsules]       = useState<{id:string;msg:string;deliverOn:string;note:string;opened:boolean}[]>(
    () => { try { return JSON.parse(localStorage.getItem('tessa-capsules')||'[]'); } catch { return []; } }
  );
  // Achievements
  const [achievements,    setAchievements]   = useState<Record<string,boolean>>(
    () => { try { return JSON.parse(localStorage.getItem('tessa-achievements')||'{}'); } catch { return {}; } }
  );
  const [showAchievements,setShowAchievements]= useState(false);
  // Tessa self-report
  const [showSelfReport,  setShowSelfReport] = useState(false);
  const [selfReport,      setSelfReport]     = useState('');
  // Ambient intelligence (typing speed / pause pattern)
  const [ambientState,    setAmbientState]   = useState<'focused'|'stressed'|'idle'|'distracted'>('idle');
  const lastKeystroke     = useRef<number>(0);
  const keystrokeGaps     = useRef<number[]>([]);
  // News ticker
  const [newsItems,       setNewsItems]      = useState<{title:string;url:string}[]>([]);
  const [showNewsTicker,  setShowNewsTicker] = useState(false);
  // Tools dock (mobile collapsed tray)
  const [showToolsDock,   setShowToolsDock]  = useState(false);
  // World clock + weather panel
  const [showWorldClock,  setShowWorldClock] = useState(false);
  // Wellness float drag position
  const [wellnessPos,     setWellnessPos]    = useState<{x:number;y:number}|null>(null);
  const wellnessDragging  = useRef(false);
  const wellnessDragStart = useRef({mx:0,my:0,px:0,py:0});
  // Formula sheet
  const [shortNotes,      setShortNotes]   = useState<{topic:string;formula:string}[]>(
    () => { try { return JSON.parse(localStorage.getItem('tessa-shortnotes')||'[]'); } catch { return []; } }
  );
  ...
  const [foodRedirectToast,         setFoodRedirectToast]          = useState(false);
  ...
  // User avatar: 
  // Creator mode → /avatars/creator-ankit.jpg (commit to public/avatars/ in GitHub repo)
  //   GitHub path: vishwajitjha79-spec repo → public/avatars/creator-ankit.jpg
  // Standard + OAuth (Google/GitHub login) → profile pic from user_metadata
  // Standard + email-only → generated initials avatar from email
  // Guest → empty (shows User icon fallback)
  const userAvatarSrc: string = isCreatorMode
    ? '/avatars/creator-ankit.jpg'  // → commit file at: public/avatars/creator-ankit.jpg
    : (user?.user_metadata?.avatar_url || user?.user_metadata?.picture)
      ? (user.user_metadata.avatar_url || user.user_metadata.picture)
      : user?.email
        ? `https://ui-avatars.com/api/?name=${encodeURIComponent(user.email.split('@')[0])}&background=6366f1&color=fff&size=128&bold=true&format=svg`
        : '';
  ...
  const setTheme = useCallback((th: Theme) => {
    ...
    if (isCreatorModePersistent()) pushCreatorSync();
  }, []);
      ...
      if (u) {
        ...
        if (isCreatorModePersistent()) {
          pullCreatorSync().then(changed => {
            if (changed) {
              setWellnessVersion(v=>v+1);
              // Re-apply all synced settings to React state
              const th = lsGet('tessa-theme') as any; if (th) setTheme(th);
              const av = lsGet('tessa-avatar'); if (av && AVATARS.find(a=>a.id===av)) setAvatarId(av);
              const pr = lsGet('tessa-persona') as any; if (pr) setTessaPersona(pr);
              const cn = lsGetJson('tessa-conversations', null); if (cn) setConversations(cn);
              const anim = lsGet('tessa-animations'); if (anim!==null) setAnimations(anim==='true');
              const sfxV = lsGet('tessa-sfx'); if (sfxV!==null) setSfx(sfxV==='true');
              const fz = lsGet('tessa-font-size') as any; if (fz) setFontSize(fz);
              const lang = lsGet('tessa-language') as any; if (lang) setLanguage(lang);
              const vp = lsGet('tessa-voice-personality') as any; if (vp) setVoicePersonality(vp);
              const sn = lsGetJson('tessa-shortnotes', null); if (sn) setShortNotes(sn);
            }
          });
        }
      }
  ...
  useEffect(() => {
    const ticker = setInterval(() => setLiveNow(new Date()), 1000);
    return () => clearInterval(ticker);
  }, []);

  // ── Haptic helper (Web Vibration API — works on Android Chrome) ───────────
  const haptic = (pattern: number | number[]) => {
    if (!hapticsOn || typeof navigator === 'undefined') return;
    try { navigator.vibrate?.(pattern); } catch {}
  };

  // ── Ambient Intelligence — monitors typing cadence ────────────────────────
  const trackKeystroke = () => {
    const now = Date.now();
    if (lastKeystroke.current > 0) {
      const gap = now - lastKeystroke.current;
      keystrokeGaps.current = [...keystrokeGaps.current.slice(-20), gap];
      const avg = keystrokeGaps.current.reduce((a,b)=>a+b,0) / keystrokeGaps.current.length;
      if      (avg < 80)  setAmbientState('focused');
      else if (avg < 200) setAmbientState('focused');
      else if (avg > 800) setAmbientState('distracted');
      else                setAmbientState('idle');
    }
    lastKeystroke.current = now;
  };

  // ── Study log — record daily activity ─────────────────────────────────────
  const recordStudyActivity = () => {
    const today = new Date().toISOString().split('T')[0];
    setStudyLog(prev => {
      const updated = { ...prev, [today]: (prev[today]||0) + 1 };
      try { localStorage.setItem('tessa-studylog', JSON.stringify(updated)); } catch {}
      return updated;
    });
  };
  ...
  const unlockAchievement = (id: string) => {
    setAchievements(prev => {
      ...
      try { localStorage.setItem('tessa-achievements', JSON.stringify(updated)); } catch {}
      haptic([50, 30, 80]);
      return updated;
    });
  };
  ...
  // ── News ticker — free RSS via allorigins proxy ────────────────────────────
  useEffect(() => {
    (async () => {
      try {
        const url = encodeURIComponent('https://feeds.bbci.co.uk/news/rss.xml');
  ...
  // ── Time capsule delivery check ────────────────────────────────────────────
  useEffect(() => {
    const today = new Date().toISOString().split('T')[0];
    const due = capsules.filter(cap => !cap.opened && cap.deliverOn <= today);
    if (due.length > 0) {
      due.forEach(cap => {
...
You wrote this on ${cap.deliverOn}:

*"${cap.msg}"*

${cap.note}`,
          timestamp: new Date(), mood: 'happy' as MoodType,
        }]);
      });
      const updated = capsules.map(cap =>
        due.find(d => d.id === cap.id) ? { ...cap, opened: true } : cap
      );
      setCapsules(updated);
      try { localStorage.setItem('tessa-capsules', JSON.stringify(updated)); } catch {}
    }
  // eslint-disable-next-line
  }, []);

  // ── Weather — fetch from free wttr.in JSON API ─────────────────────────────
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch('https://wttr.in/Delhi?format=j1');
        if (!r.ok) return;
        const d = await r.json();
        const cur = d.current_condition?.[0];
        if (!cur) return;
        ...
        const icon = code<=113?'☀️':code<=176?'🌤️':code<=266?'🌧️':code<=335?'❄️':code<=395?'⛈️':'🌫️';
        setWeather({ temp:`${temp}°C`, desc, icon, city:'Delhi' });
      } catch {}
    })();
  }, []);

  // ── Smart follow-ups — generate after each Tessa reply ────────────────────
  // ── Predictive ghost-text completion ────────────────────────────────────────
  const fetchGhostCompletion = async (partial: string) => {
    if (!partial.trim() || isLoading) return;
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          messages: [{ role: 'user', content:
            `Complete this message in 4-8 words naturally. Reply with ONLY the completion, nothing else, no quotes:
"${partial}"` }],
          isCreatorMode: false, maxTokens: 30, language: 'en',
        }),
      });
      const d = await res.json();
      const completion = d.content?.trim().replace(/^["']|["']$/g,'') || '';
      if (completion && !partial.endsWith(completion.split(' ')[0])) {
  ...
  const generateFollowUps = async (reply: string, userQ: string) => {
    try {
      const res = await fetch('/api/chat', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({
          messages:[{ role:'user', content: `Based on this conversation exchange, suggest 3 short follow-up questions the user might naturally ask next. Reply ONLY with a JSON array of 3 strings, no explanation.
User asked: "${userQ.slice(0,120)}"
Assistant replied: "${reply.slice(0,200)}"` }],
          isCreatorMode: false, maxTokens: 120, language: 'en',
        }),
      });
      const d = await res.json();
      const text = d.content?.replace(/\`\`\`json|\`\`\`/g,'').trim() || '[]';
      const arr = JSON.parse(text);
      if (Array.isArray(arr)) setFollowUps(arr.slice(0,3));
    } catch { setFollowUps([]); }
  };
  ...
  const persistConversation = useCallback(async () => {
    ...
    // Strip base64 image data before saving to avoid localStorage quota overflow
    // (base64 images can be 500KB+ each and silently truncate the JSON)
    const safeMessages = messages.map(m => ({
      ...m,
      content: typeof m.content === 'string'
        ? (m.content.startsWith('data:image') ? '[Image]' : m.content)
        : m.content,
    }));
    const conv: Conversation = {
      id:currentConvId,
      title: (safeMessages.find(m=>m.role==='user')?.content as string||'Chat').slice(0,55).trimEnd()+'…',
      messages: safeMessages, created:new Date(), updated:new Date(),
    ...
    if (user && !isGuest) {
      ...
      try {
        ...
        // Try full save; if quota exceeded, trim oldest convs and retry
        try {
          lsSet('tessa-conversations', JSON.stringify(next));
        } catch {
          const trimmed = next.map(cv => ({
            ...cv,
            messages: cv.messages.slice(-40), // keep last 40 msgs per conv if storage is tight
          }));
          lsSet('tessa-conversations', JSON.stringify(trimmed));
        }
      } catch {}
    }
...
Style: Direct, warm, specific. No generic advice. Use actual numbers from his data. Max 200 words total. No fluff.`;
  ...
  // ── Send message ──────────────────────────────────────────────────────────
  // Food/calorie questions in main chat → redirect to Health Pulse (creator only)
  const FOOD_REDIRECT_RE = /\b(ate|eat|eating|had|meal|breakfast|lunch|dinner|snack|calories?|roti|dal|rice|samosa|chai|burger|pizza|bread|fruit|salad|drank|drink|water glasses?|calories? count|how many calories)\b/i;
  const sendMessage = async (override?: string) => {
    const text = (override ?? input).trim();
    if (!text && !selectedImage) return;
    if (isLoading) return;
    // Redirect food/calorie logging to Health Pulse — works in ALL modes
    if (!override && !selectedImage && FOOD_REDIRECT_RE.test(text) &&
        /\b(ate|had|eating|drank|log|track|calories?|roti|dal|rice|samosa|chai|water glass)\b/i.test(text)) {
      setInput('');
      setFoodRedirectToast(true);
      setTimeout(() => setFoodRedirectToast(false), 3500);
      return;
    }
    ...
    try {
      ...
      setFollowUps([]);
      generateFollowUps(data.content, text).catch(()=>{});
      // Reset hidden flag so new follow-ups always show for new replies
      setFollowUpsHidden(false);
      markStudy();
      recordStudyActivity();
      haptic([20, 10, 30]);
      // Confidence score — estimate based on response quality signals
      const confScore = Math.min(99, Math.max(55,
        85
        - (data.content.includes("I'm not sure") ? 15 : 0)
        - (data.content.includes("I don't know") ? 20 : 0)
        - (data.content.includes("may be incorrect") ? 10 : 0)
        + (data.searchPerformed ? 8 : 0)
        + (data.content.length > 300 ? 4 : 0)
        + Math.floor(Math.random() * 8)
      ));
      if (showConfidence) setMsgConfidence(p => ({...p, [assistantMsg.id]: confScore}));
      // Auto-extract formulas from reply
      const formulaMatches = data.content.match(/\\$[^$]+\\$|\\\[.*?\\\]|[A-Za-z]\s*=\s*[^,\.\n]{4,40}/g);
      if (formulaMatches && formulaMatches.length > 0) {
        const topic = text.slice(0,30);
        setShortNotes(prev => {
          const newF = formulaMatches.slice(0,3).map((f:string) => ({topic, formula: f.trim()}));
          const updated = [...prev, ...newF].slice(-50);
          try { localStorage.setItem('tessa-shortnotes', JSON.stringify(updated)); } catch {}
          return updated;
        });
      }
      // Achievement: message count milestones
      const totalMsgs = messages.length + 2;
      if (totalMsgs >= 10)  unlockAchievement('first_ten');
      if (totalMsgs >= 100) unlockAchievement('century');
      if (needsSearch)      unlockAchievement('web_explorer');
      if (isCreatorMode)    unlockAchievement('creator_mode');
  ...
  const speakText = (raw: string) => {
    ...
    const vpSettings = {
      warm:         { pitch:1.85, rate:1.18, volume:0.90 },
      professional: { pitch:1.40, rate:1.05, volume:0.85 },
      playful:      { pitch:2.10, rate:1.40, volume:0.92 },
      calm:         { pitch:1.60, rate:0.95, volume:0.82 },
    };
    const vp = vpSettings[voicePersonality] || vpSettings.warm;
    u.pitch = vp.pitch; u.rate = vp.rate; u.volume = vp.volume; u.lang = lang;
    const speak = () => {
  ...
  const endVoiceCall = useCallback(() => {
    // 1. Kill loop flags FIRST — any in-flight async callback checks these
    callLoopRef.current   = false;
    isSpeakingRef.current = false;
    // 2. Null all recognition handlers BEFORE abort to prevent ghost onend firing
    ...
    // 3. Nuke speech synthesis — 5x cancel for stubborn Android Chrome lag
    for (let i = 0; i < 5; i++) try { window.speechSynthesis?.cancel(); } catch {}
    // 4. Kill timer
    if (callTimerRef.current) { clearInterval(callTimerRef.current); callTimerRef.current = null; }
    callMessagesRef.current = [];
    // 5. State — force everything to off in one flush
    ...
    setCallStatus('idle');
    setCallDuration(0);
    // 6. Safety net: re-check at 300ms and 800ms — catches lagged TTS callbacks
    [300, 800].forEach(ms => setTimeout(() => {
      if (!callLoopRef.current) {
        for (let i = 0; i < 3; i++) try { window.speechSynthesis?.cancel(); } catch {}
        setVoiceCallOpen(false);
        setCallStatus('idle');
        setIsRecording(false);
      }
  ...
  const startVoiceCall = () => {
    ...
    if (!SR) { alert('Voice calls need Chrome or Edge.'); return; }
    const isMedian = typeof (window as any).gonative !== 'undefined' || /gonative|GoNative/i.test(navigator.userAgent || '');
    const doStart = () => {
      navigator.mediaDevices.getUserMedia({ audio: true }).then(() => {
        try {
          ...
          [0,0.22].forEach(d=>{
            ...
            g.gain.linearRampToValueAtTime(0.08,ctx.currentTime+d+0.04);
        ...
        if (window.innerWidth < 640) setCallFullscreen(true);
        setCallTranscript([]);
        setCallDuration(0);
        ...
        const greet = isCreatorMode
          ? [h<6?"It's late Ankit… everything okay?":h<12?"Good morning! Ready for the day?":h<17?"Hey! What are we working on?":h<21?"Good evening Ankit! What's up?":"Getting late… but I'm here. What's up?","Hey! Missed you. What do you need?","Hi! All ears — what's going on?"][Math.floor(Math.random()*3)]
          : ["Hi! I'm Tessa. How can I help?","Hello! What's on your mind?","Hey there!"][Math.floor(Math.random()*3)];
        callMessagesRef.current.push({ role: 'assistant', content: greet });
        callSpeak(greet, () => { if (callLoopRef.current) callListen(); });
      }).catch(() => {
        if (isMedian) alert('Mic permission denied.\n\nSettings → TESSA → Permissions → Microphone → Allow');
        else alert('Mic access denied. Click the 🔒 in your browser address bar → Microphone → Allow');
      });
    };
    if (isMedian) {
      const gn = (window as any).gonative;
      if (gn?.permissions?.requestMicrophonePermission) {
        gn.permissions.requestMicrophonePermission({ callback: (r:any) => { if (r?.status==='authorized'||r?.granted) doStart(); else alert('Mic denied.'); } });
      } else { doStart(); }
    } else { doStart(); }
  };

  // ─── speak helper — mobile-hardened ────────────────────────────────────────
  const callSpeak = (text: string, onDone: () => void) => {
    if (!callLoopRef.current) { onDone(); return; }
    if (!('speechSynthesis' in window)) { onDone(); return; }

    // ── STEP 1: Hard-cancel any existing utterance ─────────────────────────
    ...
    const clean = text
      .replace(/[*_~`#>\[\]]/g,'')
      .replace(/TESSA/gi,'Tessa')
      .replace(/\n+/g,' ')
      .replace(/\s{2,}/g,' ')
      .trim()
      .slice(0, 180); // shorter = faster TTS start on mobile

    const lang = language === 'hi' ? 'hi-IN' : 'en-US';

    // ── STEP 2: Pick best voice ─────────────────────────────────────────────
    const pickVoice = (voices: SpeechSynthesisVoice[]): SpeechSynthesisVoice | null => {
      if (!voices.length) return null;
      const langVoices = voices.filter(v => v.lang.startsWith(lang === 'hi-IN' ? 'hi' : 'en'));
      // Female first
      const female = langVoices.find(v => /zira|samantha|karen|moira|tessa|veena|google us english female/i.test(v.name))
        ?? langVoices.find(v => /female|woman/i.test(v.name))
        ?? langVoices.find(v => /google.*english/i.test(v.name))
        ?? langVoices[0]
        ?? voices[0];
      return female;
    };

    // ── STEP 3: doSpeak — the actual speak call ─────────────────────────────
    const doSpeak = (voices: SpeechSynthesisVoice[]) => {
      if (!callLoopRef.current) { isSpeakingRef.current = false; return; }
      ...
      const finish = () => {
        ...
        isSpeakingRef.current = false;
        // Small gap before listening again — helps mobile recognition restart
        setTimeout(onDone, 120);
      };

      // Android Chrome keep-alive: it suspends synthesis mid-sentence
      const kaInterval = setInterval(() => {
        try {
          if (window.speechSynthesis.paused) {
            window.speechSynthesis.resume();
          }
          // Extra nudge: if it stopped speaking but onend never fired (Android bug)
          if (!window.speechSynthesis.speaking && !window.speechSynthesis.pending) {
            // already done — onend should fire, but if not we handle via hardTimer
          }
        } catch {}
      }, 200);

      // Hard timeout: if TTS hangs (common on Android for >5s), proceed anyway
      const maxMs = Math.max(4000, clean.length * 65); // ~65ms per char
      ...
      utter.onstart = () => {
        // Reset hard timer on actual start (it took longer than expected to begin)
        // We don't reset here to keep the safety net active throughout
      };
      utter.onend   = () => finish();
      utter.onerror = (ev: SpeechSynthesisErrorEvent) => {
        // 'interrupted' = barge-in or cancel() call — both are normal, continue
        // 'not-allowed', 'synthesis-failed' = real error
        if (ev.error === 'synthesis-failed' || ev.error === 'text-too-long') {
      ...
      try {
        ...
        // iOS Safari fix: speak() needs to be called within a user gesture context.
        // If it silently fails (nothing in queue after 400ms), call onDone to continue.
        setTimeout(() => {
          if (!window.speechSynthesis.speaking && !window.speechSynthesis.pending && !done) {
            // TTS failed silently — just continue the loop
            finish();
          }
        }, 600);
      } catch {
    ...
    // ── STEP 4: Get voices — Android often has them; poll if not ───────────
    const voices = window.speechSynthesis.getVoices();
    if (voices.length > 0) {
      // Small async gap lets the previous cancel() complete on Android
      setTimeout(() => doSpeak(voices), 80);
    } else {
      // Poll up to 15 × 80ms = 1.2s for voices to load
      let attempts = 0;
      const poll = setInterval(() => {
        attempts++;
        const v2 = window.speechSynthesis.getVoices();
        if (v2.length > 0 || attempts >= 15) {
          clearInterval(poll);
          window.speechSynthesis.onvoiceschanged = null;
          setTimeout(() => doSpeak(v2.length > 0 ? v2 : []), 80);
        }
      }, 80);
  ...
  const callListen = () => {
    ...
    // Abort any previous recognition instance cleanly
    if (recognitionRef.current) {
      const old = recognitionRef.current;
      recognitionRef.current = null;
      try { old.onend = null; old.onerror = null; old.abort(); } catch {}
    }

    // Short gap after TTS ends before starting recognition
    // (prevents Chrome Android from immediately picking up TTS audio as speech)
    const startRecognition = () => {
      if (!callLoopRef.current) return;

      const r = new SR();
      r.lang            = language === 'hi' ? 'hi-IN' : language === 'hinglish' ? 'hi-IN' : 'en-US';
      r.continuous      = false;    // mobile-safe: single utterance mode
      ...
      // Safety timeout — if we hear nothing for 8s, retry rather than hang
      const noInputTimer = setTimeout(() => {
        try { r.abort(); } catch {}
        // Don't increment retry count — just silently restart
        if (callLoopRef.current) setTimeout(callListen, 300);
      }, 8000);
      ...
      r.onresult = (e: any) => {
        ...
        setInterimText(interim || finalText.trim());
      };

      r.onspeechend = () => {
        // User stopped speaking — small grace period for trailing words
      ...
      r.onend = async () => {
        ...
        // Empty result (just a beep / background noise) — restart listening
        if (!said || said.length < 2) {
          setTimeout(callListen, 250);
          return;
        }
        ...
        if (callMessagesRef.current.length > 14) {
          callMessagesRef.current = callMessagesRef.current.slice(-14);
        }

        try {
          const ac  = new AbortController();
          const tmo = setTimeout(() => ac.abort(), 12000);
          const res = await fetch('/api/chat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            signal: ac.signal,
            body: JSON.stringify({
              ...
              maxTokens: 55,
              _systemOverride: `Voice call mode. Reply in ONE very short sentence (max 15 words). Plain text only — no markdown, no lists. Be warm and natural.${isCreatorMode ? " You're Ankit's personal AI companion." : ''}`,
            }),
          });
          clearTimeout(tmo);
          if (!callLoopRef.current) return;
          const data  = await res.json();
          const reply = (data.content || 'Say that again?')
            .replace(/[*_~`#>\[\]]/g, '')
            .replace(/\n+/g, ' ')
            .trim()
            .slice(0, 160);
          ...
          callSpeak(reply, () => {
            if (callLoopRef.current) setTimeout(callListen, 150);
          });
        } catch {
          if (!callLoopRef.current) return;
          callSpeak('Lost signal for a moment!', () => {
            if (callLoopRef.current) setTimeout(callListen, 800);
          });
        }
      };
      ...
      r.onerror = (e: any) => {
        ...
        setIsRecording(false);
        recognitionRef.current = null;
        // Fatal errors — end call
        if (e.error === 'not-allowed' || e.error === 'audio-capture' || e.error === 'service-not-allowed') {
          endVoiceCall(); return;
        }
        // Non-fatal: no-speech, aborted, network — restart
        if (callLoopRef.current) setTimeout(callListen, e.error === 'network' ? 1200 : 350);
      };
      ...
      try {
        ...
        // Already started or another error — retry after delay
        if (callLoopRef.current) setTimeout(callListen, 600);
      }
    };

    // Delay start slightly after TTS to avoid echo pickup
  ...
  // ── VoiceCallWindow ─────────────────────────────────────────────────────────
  // Declared with useCallback so it never re-mounts and kills the recognition loop
  const VoiceCallWindow = () => {
    if (!voiceCallOpen) return null;
    const fmt = (s:number) => `${Math.floor(s/60)}:${String(s%60).padStart(2,'0')}`;
    const sc = callStatus==='listening'?'#22c55e':callStatus==='thinking'?'#a78bfa':callStatus==='speaking'?'#f59e0b':'#64748b';
    const sl = callStatus==='listening'?'Listening':callStatus==='thinking'?'Thinking':callStatus==='speaking'?'Speaking':'Connected';
    const isActive = callStatus==='speaking'||callStatus==='listening';
    // 24-bar fixed pattern — never recomputed
    const BARS = [4,7,11,18,26,32,28,20,13,8,5,8,15,24,30,24,16,9,6,9,17,26,20,12];
      ...
      <>
        ...
        {callFullscreen && (
          <div style={{position:'fixed',inset:0,zIndex:89,
            background:'linear-gradient(160deg,#04030e 0%,#0a0620 50%,#050c1e 100%)'}} aria-hidden>
            <div style={{position:'absolute',top:'12%',left:'50%',
              transform:'translateX(-50%)',width:460,height:460,borderRadius:'50%',
              background:`radial-gradient(circle,${sc}12 0%,${sc}05 45%,transparent 70%)`,
              transition:'background 1.4s ease'}}/>
          </div>
        )}

        {/* ── Draggable mini window (non-fullscreen) ── */}
        {!callFullscreen && (
          <div
            style={{position:'fixed',zIndex:90,
              left:(callPos?.x??Math.max(16,typeof window!=='undefined'?window.innerWidth/2-150:100)),
              top:(callPos?.y??72),
              width:290,borderRadius:24,overflow:'hidden',
              background:'linear-gradient(155deg,#0e0b22 0%,#141028 100%)',
            ...
            onPointerDown={e=>{
              ...
              callDragStart.current={mx:e.clientX,my:e.clientY,
                px:callPos?.x??Math.max(16,window.innerWidth/2-150),
                py:callPos?.y??72};
            ...
            onPointerMove={e=>{
              ...
              const nx=Math.max(8,Math.min(window.innerWidth-298,callDragStart.current.px+e.clientX-callDragStart.current.mx));
              const ny=Math.max(8,Math.min(window.innerHeight-300,callDragStart.current.py+e.clientY-callDragStart.current.my));
              (e.currentTarget as HTMLElement).style.left=nx+'px';
              (e.currentTarget as HTMLElement).style.top=ny+'px';
            }}
            onPointerUp={e=>{
              if(!callDragging.current)return;
            ...
            onPointerCancel={()=>{callDragging.current=false;}}
          >
            {/* Mini top bar */}
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 14px 8px'}}>
              <div style={{display:'flex',alignItems:'center',gap:6}}>
                ...
                <span style={{fontSize:10,fontWeight:700,color:sc,letterSpacing:'0.08em',textTransform:'uppercase'}}>{sl}</span>
              </div>
              <span style={{fontSize:10,color:'rgba(255,255,255,0.28)',fontVariantNumeric:'tabular-nums'}}>{fmt(callDuration)}</span>
              <button onClick={()=>setCallFullscreen(true)}
                style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',
                  borderRadius:6,padding:'3px 8px',color:'rgba(255,255,255,0.4)',fontSize:9,cursor:'pointer',
                  transition:'none'}}>
                ⛶ Full
              </button>
            </div>
            ...
            {/* Mini avatar */}
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'0 16px 16px',gap:8}}>
              <div style={{position:'relative',width:64,height:64,borderRadius:'50%',overflow:'hidden',
                border:`2px solid ${sc}`,boxShadow:`0 0 16px ${sc}35`,
                transition:'border-color 0.6s,box-shadow 0.6s'}}>
                <img src={avatarSrc} alt="Tessa" style={{width:'100%',height:'100%',objectFit:'cover'}}
                  onError={e=>{(e.currentTarget as HTMLImageElement).src='/avatars/cosmic.png';}}/>
              </div>
              <p style={{color:'#fff',fontWeight:800,fontSize:13,letterSpacing:'0.06em',margin:0}}>TESSA</p>

              {/* Mini waveform — pure CSS scaleY, no height change */}
              <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:3,height:24,width:'100%'}}>
                {BARS.slice(0,20).map((h,i)=>(
                  <div key={i} style={{
                    width:3,height:h*0.6,borderRadius:2,
                    background:isActive?sc:'rgba(255,255,255,0.12)',
                    transform:isActive?'scaleY(1)':'scaleY(0.2)',
                    transformOrigin:'center',
                    animation:isActive?`callBar ${0.7+Math.sin(i*1.2)*0.35}s ease-in-out infinite alternate`:'none',
              ...
              {/* Mini transcript last line */}
              {(callTranscript.length>0||interimText) && (
                <div style={{width:'100%',borderRadius:10,background:'rgba(255,255,255,0.04)',
                  padding:'5px 8px',minHeight:24}}>
                  {interimText
                    ? <p style={{fontSize:9,color:sc,fontStyle:'italic',margin:0,lineHeight:1.4}}>{interimText}…</p>
                    : callTranscript.length>0 && (
                      <p style={{fontSize:9,color:'rgba(255,255,255,0.55)',margin:0,lineHeight:1.4}}>
                        {callTranscript[callTranscript.length-1].text.slice(0,80)}
                      </p>
                    )
                  }
                </div>
              ...
              <button onClick={endVoiceCall}
                style={{width:48,height:48,borderRadius:'50%',border:'none',cursor:'pointer',
                  background:'linear-gradient(135deg,#ff3b30,#c90010)',
              ...
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="white">
                  <path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.27-.27.7-.36 1.06-.2 1.1.45 2.3.7 3.54.7.55 0 1 .45 1 1V20c0 .55-.45 1-1 1C7.6 21 3 16.4 3 11c0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.6 3.56.14.35.05.77-.22 1.04L6.6 10.8z" transform="rotate(135 12 12)"/>
                </svg>
              </button>
            </div>
          </div>
          ...
          <div style={{position:'fixed',inset:0,zIndex:90,
            ...
            <div style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <div style={{width:8,height:8,borderRadius:'50%',background:sc,
                  boxShadow:`0 0 8px ${sc}`,transition:'background 0.6s'}}/>
                <span style={{fontSize:11,fontWeight:700,color:sc,letterSpacing:'0.1em',
                  textTransform:'uppercase',transition:'color 0.6s'}}>{sl}</span>
              </div>
              <span style={{fontSize:12,color:'rgba(255,255,255,0.28)',fontVariantNumeric:'tabular-nums'}}>
                {fmt(callDuration)}
              </span>
              <button onClick={()=>setCallFullscreen(false)}
                style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.1)',
                  borderRadius:8,padding:'5px 12px',color:'rgba(255,255,255,0.45)',
                  fontSize:10,cursor:'pointer',letterSpacing:'0.04em',transition:'none'}}>
                ⊡ Window
              </button>
            </div>
            ...
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:12,flexShrink:0}}>
              <div style={{position:'relative',display:'flex',alignItems:'center',justifyContent:'center'}}>
                {/* Rings — CSS animation only, will-change compositor */}
                {[108,144,180].map((sz,ri)=>(
                  <div key={ri} style={{
                    position:'absolute',width:sz,height:sz,borderRadius:'50%',
                    border:`1px solid ${sc}`,
                    opacity:isActive?[0.4,0.2,0.08][ri]:0.04,
                    animation:isActive?`callRingPulse ${2+ri*0.6}s ease-out infinite`:'none',
                    animationDelay:`${ri*0.6}s`,
                    transition:'opacity 1s,border-color 0.8s',
                    willChange:'transform,opacity',
                  }}/>
                ))}
                <div style={{width:96,height:96,borderRadius:'50%',overflow:'hidden',
                  border:`2.5px solid ${sc}`,
                  boxShadow:`0 0 0 3px ${sc}14,0 0 32px ${sc}40`,
                  transition:'border-color 0.6s,box-shadow 0.6s',zIndex:1}}>
                  <img src={avatarSrc} alt="Tessa"
                    style={{width:'100%',height:'100%',objectFit:'cover'}}
                    onError={e=>{(e.currentTarget as HTMLImageElement).src='/avatars/cosmic.png';}}/>
                </div>
              </div>
              <div style={{textAlign:'center'}}>
                <p style={{color:'#fff',fontWeight:900,fontSize:20,letterSpacing:'0.06em',margin:'0 0 3px'}}>TESSA</p>
                <p style={{color:'rgba(255,255,255,0.28)',fontSize:11,margin:0}}>
                  {isCreatorMode?'💝 Personal AI':'AI Voice Call'}
                </p>
              </div>
            </div>

            {/* Fullscreen waveform — pure CSS, no JS height changes */}
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',
              gap:4,height:56,width:'100%',padding:'0 24px',flexShrink:0}}>
              {BARS.map((h,i)=>(
                <div key={i} style={{
                  width:5,height:Math.max(5,h),borderRadius:3,
                  background:isActive?sc:'rgba(255,255,255,0.08)',
            ...
            <div style={{width:'100%',maxWidth:460,flexShrink:0,
              height:148,overflowY:'auto',borderRadius:16,
              background:'rgba(255,255,255,0.05)',
              border:'1px solid rgba(255,255,255,0.07)',
              padding:'10px 14px',
              overscrollBehavior:'contain'}}>
              {callTranscript.length===0 && !interimText
                ? <p style={{fontSize:12,color:'rgba(255,255,255,0.2)',textAlign:'center',margin:'20px 0'}}>
                    Conversation appears here…
                  </p>
                : callTranscript.slice(-7).map((m,i)=>(
                  <div key={i} style={{marginBottom:8}}>
                    <span style={{fontSize:9,fontWeight:800,letterSpacing:'0.08em',
                      textTransform:'uppercase',color:m.role==='user'?'rgba(255,255,255,0.35)':sc,
                      display:'block',marginBottom:2}}>
                      {m.role==='user'?'You':'Tessa'}
                    </span>
                    <p style={{fontSize:13,color:m.role==='user'?'rgba(255,255,255,0.75)':'#fff',
                      margin:0,lineHeight:1.5,fontWeight:m.role==='tessa'?500:400}}>
                      {m.text}
                    </p>
                  </div>
                ))
              ...
              {interimText && (
                <p style={{fontSize:12,color:sc,fontStyle:'italic',margin:'4px 0 0',opacity:0.8}}>
                  {interimText}…
                </p>
              )}
            </div>
            ...
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:8,flexShrink:0}}>
              <p style={{fontSize:11,fontWeight:600,letterSpacing:'0.06em',textTransform:'uppercase',
                color:isActive?sc:'rgba(255,255,255,0.2)',margin:0,transition:'color 0.5s'}}>
                {callStatus==='listening'?'🎤 Listening':callStatus==='thinking'?'🧠 Thinking…':callStatus==='speaking'?'🔊 Speaking':''}
              </p>
              <button onClick={endVoiceCall}
                style={{width:64,height:64,borderRadius:'50%',border:'none',cursor:'pointer',
                  background:'linear-gradient(135deg,#ff3b30,#c90010)',
                  boxShadow:'0 8px 28px rgba(255,59,48,0.5),0 0 0 4px rgba(255,59,48,0.12)',
                  display:'flex',alignItems:'center',justifyContent:'center',
              ...
              <p style={{color:'rgba(255,255,255,0.15)',fontSize:9,margin:0,letterSpacing:'0.05em'}}>TAP TO END</p>
            </div>
          </div>
        )}
      </>
  ...
  return (
    <div className={`h-screen ${t.text} flex overflow-hidden relative ${fontSizeClass}`}
      ...
      /* Waveform bars — pure scaleY on compositor layer, no layout */
      @keyframes callBar {
        0%   { transform: scaleY(0.15); }
        50%  { transform: scaleY(1.0);  }
        100% { transform: scaleY(0.22); }
      }
      /* Avatar rings — slow organic pulse */
      @keyframes callRingPulse {
        0%   { transform: scale(0.92); opacity: 0.6; }
        70%  { transform: scale(1.18); opacity: 0.1; }
        100% { transform: scale(1.28); opacity: 0;   }
      }
      .soundbar-wrap { display:flex; align-items:flex-end; }
      @keyframes spin {
      ...
      @media (max-width: 767px) {
        /* Nuclear option: kill every blur/filter — #1 GPU killer */
        * { backdrop-filter: none !important; -webkit-backdrop-filter: none !important;
            filter: none !important; }
        /* Contain layout per message to avoid full-list reflow */
        .msg-row { contain: layout style paint; }
        /* No expensive shadows on bubbles */
        .msg-bubble { box-shadow: none !important; filter: none !important; }
        /* Snap transitions to near-instant */
        *, *::before, *::after { transition-duration: 0.05s !important;
                                  animation-duration: 0.05s !important; }
        /* Kill decorative animations entirely */
        .float-heart, .status-ping, .animate-ping { display: none !important; }
        .animate-pulse { animation: none !important; opacity: 1 !important; }
        /* Kill gradient backgrounds on messages — use flat colour */
        .msg-bubble-gradient { background-image: none !important; }
        /* EXEMPTIONS — waveform bars must animate freely */
        .wave-bar, [style*="callBar"] {
          animation-duration: initial !important;
          transition-duration: initial !important;
        }
      }

      /* ── VOICE CALL: promote each bar to its own compositor layer ── */
      .wave-bar {
...
`}</style>
      ...
      {showSettings && (
        <>
            ...
            <div className="fixed inset-x-0 bottom-0 z-50 flex flex-col rounded-t-[26px] overflow-hidden"
              ...
              <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-4 space-y-4">
                ...
                {settingsTab==='themes' && (<>
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      {(isCreatorMode
                        ? ([['dark','🌙','Dark'],['light','☀️','Light'],['cyberpunk','⚡','Cyber'],['ocean','🌊','Ocean'],['sunset','🌅','Sunset'],['pastel','🪻','Pastel'],['sakura','🌸','Sakura'],['ankit','🌿',"Ankit's Special"]] as [Theme,string,string][])
                ...
                {settingsTab==='ai' && (<>
                  ...
                  <Hr cls={t.div}/>
                  <div>
                    <SLabel label="Voice Personality" t={t}/>
                    <div className="grid grid-cols-2 gap-1.5">
                      {([
                        ['warm','🌸','Warm','Expressive & bright'],
                        ['professional','💼','Professional','Clear & measured'],
                        ['playful','🎈','Playful','Bubbly & energetic'],
                        ['calm','🌊','Calm','Slow & soothing'],
                      ] as [string,string,string,string][]).map(([id,ico,label,desc])=>(
                        <button key={id} onClick={()=>setVoicePersonality(id as any)}
                          className="flex flex-col items-start p-2.5 rounded-xl transition-all active:scale-98 text-left"
                          style={{
                            background: voicePersonality===id ? `${t.glow}15` : t.isLight?'#f8fafc':'#1c2333',
                            border: `1.5px solid ${voicePersonality===id ? t.glow+'50' : t.glow+'14'}`,
                          }}>
                          <span style={{fontSize:18}}>{ico}</span>
                          <p className="text-[11px] font-bold mt-1" style={{color:voicePersonality===id?t.glow:undefined}}>{label}</p>
                          <p className={`text-[9px] mt-0.5 ${t.sSub}`}>{desc}</p>
                        </button>
                      ))}
                    </div>
                  </div>
                  <Hr cls={t.div}/>
                  <div>
                    <SLabel label="Tessa Persona" t={t}/>
                    <div className="space-y-1.5">
                      {([
                        ['companion','💝','Companion','Warm, caring personal AI'],
                        ['mentor','🎓','Mentor','Direct, guides you forward'],
                        ['studybuddy','📚','Study Buddy','Focused & academic'],
                        ['therapist','🧘','Therapist','Calm, emotionally aware'],
                        ['debater','⚡','Debater','Challenges your thinking'],
                      ] as [string,string,string,string][]).map(([id,ico,label,desc])=>(
                        <button key={id} onClick={()=>setTessaPersona(id as any)}
                          className="w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all active:scale-98 text-left"
                          style={{
                            background: tessaPersona===id ? `${t.glow}12` : t.isLight?'#f8fafc':'#1c2333',
                            border: `1px solid ${tessaPersona===id ? t.glow+'40' : t.glow+'12'}`,
                          }}>
                          <span style={{fontSize:18,flexShrink:0}}>{ico}</span>
                          <div className="flex-1 min-w-0">
                            <p className="text-[11px] font-bold" style={{color:tessaPersona===id?t.glow:undefined}}>{label}</p>
                            <p className={`text-[9px] ${t.sSub}`}>{desc}</p>
                          </div>
                          {tessaPersona===id && <Check size={11} className="flex-shrink-0" style={{color:t.glow}}/>}
                        </button>
                      ))}
                    </div>
                    <p className={`text-[9px] mt-1.5 px-1 ${t.sSub} opacity-60`}>Shapes Tessa's tone in every reply</p>
                  </div>
                </>)}
                ...
                {settingsTab==='about' && (<>
                  <div className="text-center py-2">
                    ...
                    <div className="mt-2 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold"
                      ...
                      <Sparkles size={9}/>v8.0 · AI-Powered · 10 Avatars
                    </div>
                  </div>
                  <Hr cls={t.div}/>
                  <div className="space-y-1.5">
                  ...
                  <div>
                    <SLabel label="What's New" t={t}/>
                    <button onClick={()=>{setShowSettings(false);setShowChangelog(true);}}
                      className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all active:scale-98"
                      style={{background:`${t.glow}0a`,border:`1px solid ${t.glow}22`}}>
                      <div className="flex items-center gap-2">
                        <span style={{fontSize:16}}>🚀</span>
                        <div className="text-left">
                          <p className="text-[11px] font-bold" style={{color:t.glow}}>v8.0 Release Notes</p>
                          <p className={`text-[9px] ${t.sSub}`}>15 new features · Mar 2026</p>
                        </div>
                      </div>
                      <ChevronRight size={14} className={t.sSub}/>
                    </button>
                  </div>
                </>)}
          ...
          {showAvatarPickerInSettings && (
            <AvatarPickerModal
              current={avatarId}
              onSelect={(id) => { setAvatarId(id); lsSet('tessa-avatar', id); if(isCreatorMode) { setSyncStatus('syncing'); pushCreatorSync().then(()=>{setSyncStatus('synced');setTimeout(()=>setSyncStatus('idle'),2000);}); } }}
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0 relative z-10" style={{height:"100dvh"}}>
        ...
        {/* ── NEWS TICKER ── */}
        {showNewsTicker && newsItems.length > 0 && (
          <div className="flex-shrink-0 overflow-hidden"
            style={{background:t.isLight?'#1e293b':'#0a0a12',borderBottom:`1px solid ${t.glow}20`,height:28}}>
            <div className="flex items-center h-full">
              <span className="flex-shrink-0 px-3 text-[9px] font-black tracking-widest uppercase"
                style={{color:t.glow,borderRight:`1px solid ${t.glow}25`}}>LIVE NEWS</span>
              <div className="flex-1 overflow-hidden relative">
                <div className="ticker-scroll flex items-center gap-8 h-full"
                  style={{animation:'tickerScroll 60s linear infinite',whiteSpace:'nowrap'}}>
                  {[...newsItems,...newsItems].map((item,i)=>(
                    <button key={i} onClick={()=>{setInput(`Tell me about: ${item.title}`);setShowNewsTicker(false);}}
                      className="text-[9px] font-medium text-white/60 hover:text-white transition-colors flex-shrink-0 flex items-center gap-1.5">
                      <span style={{color:t.glow}}>◆</span>{item.title}
                    </button>
                  ))}
                </div>
              </div>
              <button onClick={()=>setShowNewsTicker(false)}
                className="flex-shrink-0 px-2 text-white/30 hover:text-white/60 text-[10px]">✕</button>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center justify-between gap-2 h-[60px]">
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                {ambientState!=='idle' && (
                  <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[8px] font-bold mt-0.5"
                    style={{
                      background: ambientState==='focused'?`${t.glow}18`:ambientState==='stressed'?'rgba(239,68,68,0.15)':'rgba(148,163,184,0.15)',
                      color: ambientState==='focused'?t.glow:ambientState==='stressed'?'#ef4444':'#94a3b8',
                    }}>
                    {ambientState==='focused'?'⚡ Focused':ambientState==='stressed'?'⚠ Stressed':'💭 Drifting'}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-0.5 flex-shrink-0">
              ...
              <div className="hidden md:flex items-center gap-0.5">
                ...
                {weather && (
                  <div className="hidden md:flex items-center gap-1 px-2 py-1 rounded-lg"
                    style={{background:`${t.glow}0a`,border:`1px solid ${t.glow}18`}}
                    title={weather.desc}>
                    <span style={{fontSize:14}}>{weather.icon}</span>
                    <span className="text-[10px] font-semibold tabular-nums" style={{color:t.glow}}>{weather.temp}</span>
                  </div>
                )}
              ...
              {/* ── MOBILE: theme + settings always; ⋯ only in creator mode ── */}
        ...
        {/* ── WELLNESS FLOATING PANEL — draggable ── */}
        {showWellness && isCreatorMode && (
          <>
            ...
            <div className="fixed z-[49]"
              style={{
                bottom: wellnessPos ? undefined : 76,
                left: wellnessPos ? undefined : '50%',
                top: wellnessPos ? wellnessPos.y : undefined,
                transform: wellnessPos ? `translate(${wellnessPos.x}px,0)` : 'translateX(-50%)',
                width:'min(480px,94vw)',
                animation: wellnessPos ? 'none' : 'floatIn 0.28s cubic-bezier(0.22,1,0.36,1)',
                willChange:'transform',
                cursor:'grab',
              }}
              onPointerDown={(e)=>{
                if((e.target as HTMLElement).closest('button,input,textarea,select')) return;
                wellnessDragging.current=true;
                const rect=(e.currentTarget as HTMLElement).getBoundingClientRect();
                wellnessDragStart.current={mx:e.clientX,my:e.clientY,px:rect.left,py:rect.top};
                (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId);
              }}
              onPointerMove={(e)=>{
                if(!wellnessDragging.current) return;
                const nx=wellnessDragStart.current.px+(e.clientX-wellnessDragStart.current.mx);
                const ny=wellnessDragStart.current.py+(e.clientY-wellnessDragStart.current.my);
                (e.currentTarget as HTMLElement).style.left=nx+'px';
                (e.currentTarget as HTMLElement).style.top=ny+'px';
                (e.currentTarget as HTMLElement).style.bottom='auto';
                (e.currentTarget as HTMLElement).style.transform='none';
              }}
              onPointerUp={(e)=>{
                if(!wellnessDragging.current) return;
                wellnessDragging.current=false;
                const nx=wellnessDragStart.current.px+(e.clientX-wellnessDragStart.current.mx);
                const ny=wellnessDragStart.current.py+(e.clientY-wellnessDragStart.current.my);
                setWellnessPos({x:nx,y:ny});
              }}>
        ...
        {/* ── MESSAGES AREA ──────────────────────────────────────────────── */}
        <div className="flex-1 overflow-y-auto px-3 md:px-8 py-4 scroll-smooth">
              ...
              <div className="space-y-2.5 pb-2">
                ...
                {messages.map((msg,idx)=>{
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`msg-row group flex gap-2 ${isUser?'justify-end':'justify-start'} ${isGrouped?'mt-1':'mt-3'}`}
                    >
                      {/* LEFT: Tessa avatar icon — circular, clean */}
                      {!isUser && (
                        <div className="flex-shrink-0 flex flex-col justify-end" style={{width:34}}>
                          {!isGrouped ? (
                            <div style={{
                              width:32,height:32,borderRadius:'50%',overflow:'hidden',
                              flexShrink:0,marginBottom:3,
                              border:`1.5px solid ${t.glow}45`,
                              boxShadow:animations?`0 2px 10px ${t.glow}25`:'none',
                            }}>
                              <img src={avatarSrc} alt="Tessa"
                      ...
                      {/* BUBBLE */}
                      <div className={`flex flex-col ${isUser?'items-end max-w-[92%]':'items-start max-w-[92%]'}`}>
                        ...
                        {!isGrouped && (
                          <div className={`flex items-center gap-1.5 mb-1 px-0.5 ${isUser?'flex-row-reverse':''}`}>
                            ...
                            {!isUser && showConfidence && msgConfidence[msg.id] && (
                              <span className="text-[8px] px-1.5 py-0.5 rounded-full font-bold tabular-nums"
                                style={{
                                  background: msgConfidence[msg.id]>=85?`${t.glow}18`:msgConfidence[msg.id]>=70?'rgba(251,191,36,0.15)':'rgba(239,68,68,0.12)',
                                  color: msgConfidence[msg.id]>=85?t.glow:msgConfidence[msg.id]>=70?'#f59e0b':'#ef4444',
                                  border: `1px solid ${msgConfidence[msg.id]>=85?t.glow+'30':msgConfidence[msg.id]>=70?'#f59e0b30':'#ef444430'}`,
                                }}>
                                {msgConfidence[msg.id]}% conf
                              </span>
                            )}
                          </div>
                        )}
                        ...
                        {/* ── Reaction display + picker trigger ── */}
                        <div className="flex items-center gap-1 mt-1 px-0.5">
                          {msgReactions[msg.id] && (
                            <button onClick={()=>setMsgReactions(p=>({...p,[msg.id]:''}))}
                          ...
                          <button
                            onClick={()=>setShowReactPicker(p=>p===msg.id?null:msg.id)}
                            className="opacity-0 group-hover:opacity-100 transition-opacity text-[11px] px-1.5 py-0.5 rounded-full hover:scale-110"
                            style={{background:`${t.glow}12`,border:`1px solid ${t.glow}20`,color:t.glow}}
                            title="React">
                            {msgReactions[msg.id] ? '✏️' : '+'}
                          </button>
                          {showReactPicker===msg.id && (
                            <div className="flex gap-1 px-2 py-1.5 rounded-2xl shadow-lg z-10"
                              style={{background:t.isLight?'#fff':'#1e293b',border:`1px solid ${t.glow}25`,boxShadow:`0 4px 20px ${t.glow}20`}}>
                              {['❤️','😂','👍','🔥','💡','😮','😢','🎉'].map(e=>(
                                <button key={e} onClick={()=>{setMsgReactions(p=>({...p,[msg.id]:e}));setShowReactPicker(null);}}
                      ...
                      {/* RIGHT: User avatar — GitHub (creator) or Google/Gravatar (standard) */}
                      {isUser && (
                        <div className="flex-shrink-0 flex flex-col justify-end" style={{width:34}}>
                          {!isGrouped && (
                            <div style={{width:32,height:32,borderRadius:'50%',overflow:'hidden',
                              border:`1.5px solid ${t.glow}40`,
                              boxShadow:animations?`0 2px 8px ${t.glow}20`:'none',
                              flexShrink:0,marginBottom:3,
                              background:`linear-gradient(135deg,${t.glow}40,${glow2}30)`,
                              display:'flex',alignItems:'center',justifyContent:'center',
                            }}>
                              {userAvatarSrc
                                ? <img src={userAvatarSrc} alt="You" style={{width:'100%',height:'100%',objectFit:'cover'}}
          ...
          <div className={`flex-shrink-0 ${t.bar} px-3 md:px-6 py-3 safe-bottom`}>
            <div className="max-w-5xl mx-auto w-full">
              ...
              {followUps.length > 0 && !isLoading && (
                followUpsHidden ? (
                  /* Collapsed recall pill — unobtrusive, always available */
                  <div className="mb-2 flex items-center gap-1.5">
                    <button
                      onClick={()=>setFollowUpsHidden(false)}
                      className="flex items-center gap-1.5 h-6 px-2.5 rounded-full transition-all active:scale-95 hover:opacity-80"
                      style={{
                        background:`${t.glow}0a`,
                        border:`1px solid ${t.glow}22`,
                      }}
                      title="Show follow-up suggestions">
                      <span style={{color:t.glow,fontSize:9,fontWeight:700,letterSpacing:'0.06em'}}>✦</span>
                      <span className="text-[9px] font-medium" style={{color:t.glow,opacity:0.7}}>
                  ...
                  <div className="mb-2 flex flex-wrap gap-1.5 animate-in fade-in slide-in-from-bottom-2">
                    <div className="flex items-center justify-between w-full mb-0.5">
                      <span className="text-[9px] font-semibold" style={{color:t.glow,letterSpacing:'0.06em'}}>
                        ✦ FOLLOW UP
                      </span>
                      <button
                        onClick={()=>setFollowUpsHidden(true)}
                        className="text-[9px] px-2 py-0.5 rounded-full transition-all hover:opacity-80 flex items-center gap-1"
                        style={{color:t.isLight?'#94a3b8':'rgba(255,255,255,0.3)'}}
                        title="Hide suggestions (click ✦ to recall)">
                        ✕ hide
                      </button>
                    </div>
                    {followUps.map((q,i)=>(
                      <button key={i}
                        onClick={()=>{setInput(q);setFollowUpsHidden(true);setTimeout(()=>textareaRef.current?.focus(),50);}}
                        className="text-[11px] px-3 py-1.5 rounded-full transition-all active:scale-95 hover:scale-[1.02] text-left"
                        style={{
                          background:`${t.glow}0f`,
                          border:`1px solid ${t.glow}30`,
                          color:t.isLight?'#374151':'#e2e8f0',
              ...
              <div className="flex items-end gap-2">
                ...
                {/* Textarea with ghost completion overlay */}
                <div className="flex-1 relative">
                  {/* Ghost text layer — sits behind real input text */}
                  {ghostText && input.length > 8 && (
                    <div className="absolute inset-0 px-4 py-2.5 pointer-events-none overflow-hidden"
                      style={{fontSize:'inherit',lineHeight:'1.5',fontFamily:'inherit'}}>
                      <span style={{visibility:'hidden'}}>{input}</span>
                      <span style={{color:t.isLight?'#9ca3af':'#4b5563',fontStyle:'italic'}}> {ghostText}</span>
                    </div>
                  )}
                  <textarea ref={textareaRef} value={input}
                    onChange={e=>{
                      ...
                      trackKeystroke();
                      // Ghost text: only trigger when 3+ words, debounced 900ms
                      if (ghostDebounce.current) clearTimeout(ghostDebounce.current);
                      const val = e.target.value;
                      if (val.trim().split(' ').length >= 3 && val.length > 12) {
                        ghostDebounce.current = setTimeout(()=>fetchGhostCompletion(val), 900);
                      } else {
                        setGhostText('');
                      }
                    }}
              ...
              <div className="mt-2 flex items-center gap-1">
                ...
                {/* Web search — icon chip */}
                <button onClick={()=>setWebSearchOn(p=>!p)}
                  title={webSearchOn?'Web search ON — click to disable':'Enable web search'}
                  className="flex-shrink-0 flex items-center gap-1 h-7 px-2.5 rounded-lg transition-all active:scale-95"
                  style={{
                    background: webSearchOn
                      ? t.isLight ? `${t.glow}18` : `${t.glow}20`
                      : 'transparent',
                    color: webSearchOn ? t.glow : t.isLight?'#94a3b8':'rgba(255,255,255,0.32)',
                  }}>
                  <Globe size={13} strokeWidth={webSearchOn?2.2:1.8}/>
                  <span className="text-[10px] font-semibold tracking-tight" style={{color:'inherit'}}>
                    {webSearchOn?'Web ✓':'Web'}
                  </span>
                </button>

                {/* Separator */}
                <div className="flex-shrink-0 w-px h-3.5 mx-0.5 rounded-full" style={{background:t.isLight?'rgba(0,0,0,0.1)':'rgba(255,255,255,0.08)'}}/>

                {/* ── MOBILE: single "Tools" chip ── */}
                <div className="sm:hidden relative flex-shrink-0">
                  <button
                    onClick={()=>setShowToolsDock(p=>!p)}
                    className="flex items-center gap-1 h-7 px-2.5 rounded-lg transition-all active:scale-95"
                    style={{
                      background: showToolsDock
                        ? t.isLight ? `${t.glow}18` : `${t.glow}20`
                        : 'transparent',
                      color: showToolsDock ? t.glow : t.isLight?'#94a3b8':'rgba(255,255,255,0.32)',
                    }}>
                    ...
                    <span className="text-[10px] font-semibold tracking-tight">Tools</span>
                    {tessaPersona!=='companion' && (
                      <span className="w-1 h-1 rounded-full" style={{background:t.glow}}/>
                    )}
                  </button>

                  {/* Mobile tools tray — pops up above input */}
                  {showToolsDock && (
                    <>
                      <div className="fixed inset-0 z-[55]" onClick={()=>setShowToolsDock(false)}/>
                      <div className="absolute bottom-full left-0 mb-2 z-[56] rounded-2xl overflow-hidden"
                        style={{
                          minWidth:240,
                          background:t.isLight?'rgba(252,251,248,0.99)':'rgba(12,14,28,0.99)',
                          boxShadow:t.isLight
                            ? '0 -4px 24px rgba(0,0,0,0.12), 0 0 0 1px rgba(0,0,0,0.07)'
                            : '0 -4px 32px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.06)',
                        }}>
                        {/* Tray header */}
                        <div className="flex items-center justify-between px-3.5 pt-3 pb-1">
                          <span className="text-[11px] font-black" style={{color:t.isLight?'#374151':'rgba(255,255,255,0.6)'}}>Tools</span>
                          <button onClick={()=>setShowToolsDock(false)}
                            className="w-6 h-6 flex items-center justify-center rounded-lg transition-all active:scale-90"
                            style={{color:t.isLight?'#94a3b8':'rgba(255,255,255,0.3)'}}>
                            <X size={12}/>
                          </button>
                        </div>
                        {/* Grid of tools — clean, no cheap borders */}
                        <div className="p-2.5 grid grid-cols-4 gap-1">
                          {[
                            {ico:'🧠', lbl:'Quiz',    fn:()=>{setShowQuizGen(true);setShowToolsDock(false);}},
                            {ico:'🧬', lbl:'Memory',  fn:()=>{setShowMemTimeline(true);setShowToolsDock(false);}},
                            {ico:'✦',  lbl:'Persona', fn:()=>{setShowProfiles(true);setShowToolsDock(false);}, active:tessaPersona!=='companion'},
                            {ico:'📒', lbl:'Notes',   fn:()=>{setShowShortNotes(true);setShowToolsDock(false);}, badge:shortNotes.length},
                            {ico:'📬', lbl:'Capsule', fn:()=>{setShowTimeCapsule(true);setShowToolsDock(false);}},
                            {ico:'📰', lbl:'News',    fn:()=>{setShowNewsTicker(p=>!p);setShowToolsDock(false);}, active:showNewsTicker},
                            {ico:'🌍', lbl:'World',   fn:()=>{setShowWorldClock(true);setShowToolsDock(false);}},
                          ].map(item=>(
                            <button key={item.lbl}
                              onClick={item.fn}
                              className="relative flex flex-col items-center gap-1.5 py-3 px-1 rounded-xl transition-all active:scale-90"
                              style={{
                                background: item.active
                                  ? t.isLight ? `${t.glow}14` : `${t.glow}18`
                                  : 'transparent',
                              }}>
                              <span style={{fontSize:20,lineHeight:1}}>{item.ico}</span>
                              <span style={{
                                fontSize:9, fontWeight:600, textAlign:'center', lineHeight:1.2,
                                color: item.active ? t.glow : t.isLight?'#64748b':'rgba(255,255,255,0.5)',
                              }}>
                                {item.lbl}
                              </span>
                              {(item.badge??0)>0&&(
                                <span className="absolute top-1.5 right-1.5 w-3.5 h-3.5 rounded-full flex items-center justify-center text-white font-black"
                                  style={{background:t.glow,fontSize:7}}>{item.badge}</span>
                              )}
                            </button>
                          ))}
                        </div>
                ...
                {/* ── DESKTOP: clean icon chips ── */}
                <div className="hidden sm:flex items-center gap-0.5 overflow-x-auto"
                  style={{scrollbarWidth:'none'}}>
                  {[
                    {ico:'🧠', lbl:'Quiz',     fn:()=>setShowQuizGen(true)},
                    {ico:'🧬', lbl:'Memory',   fn:()=>setShowMemTimeline(true)},
                    {ico:'✦',  lbl:tessaPersona!=='companion'?tessaPersona.charAt(0).toUpperCase()+tessaPersona.slice(1):'Persona', fn:()=>setShowProfiles(true), active:tessaPersona!=='companion'},
                    {ico:'📒', lbl:'Notes',    fn:()=>setShowShortNotes(true), badge:shortNotes.length},
                    {ico:'📬', lbl:'Capsule',  fn:()=>setShowTimeCapsule(true)},
                    {ico:'📰', lbl:'News',     fn:()=>setShowNewsTicker(p=>!p), active:showNewsTicker},
                    {ico:'🌍', lbl:'World',    fn:()=>setShowWorldClock(true)},
                  ].map(btn=>(
                    <button key={btn.lbl} onClick={btn.fn}
                      className="relative flex-shrink-0 flex items-center gap-1 h-7 px-2.5 rounded-lg transition-all active:scale-95"
                      style={{
                        background: btn.active
                          ? t.isLight ? `${t.glow}16` : `${t.glow}20`
                          : 'transparent',
                        color: btn.active ? t.glow : t.isLight?'#94a3b8':'rgba(255,255,255,0.32)',
                      }}>
                      <span style={{fontSize:12,lineHeight:1,filter:'none'}}>{btn.ico}</span>
                      <span className="text-[10px] font-medium tracking-tight" style={{color:'inherit'}}>
                        {btn.lbl}
                      </span>
                      {(btn.badge??0)>0&&(
                        <span className="absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full flex items-center justify-center text-white font-black"
                          style={{background:t.glow,fontSize:6}}>{btn.badge}</span>
                      )}
                    </button>
                  ))}
                </div>
                ...
                {lastSearchData && (
                  <>
                    <div className="flex-shrink-0 w-px h-3.5 mx-0.5 rounded-full" style={{background:t.isLight?'rgba(0,0,0,0.1)':'rgba(255,255,255,0.08)'}}/>
                    <button onClick={()=>setShowSearchPanel(p=>!p)} className="flex-shrink-0 flex items-center gap-1 h-7 px-2.5 rounded-lg transition-all active:scale-95"
                      style={{
                        background: showSearchPanel ? (t.isLight?`${t.glow}16`:`${t.glow}20`) : 'transparent',
                        color: t.glow,
                      }}>
                      <ExternalLink size={11} strokeWidth={2}/>
                      <span className="text-[10px] font-semibold tracking-tight">{lastSearchData.sources.length} sources</span>
                    </button>
                  </>
                )}
                ...
                {isRecording && (
                  <span className="flex-shrink-0 text-[9px] font-medium text-emerald-400 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"/>Listening
                  </span>
                )}
                {isCreatorMode && !isRecording && (
                  <span className="flex-shrink-0 text-[9px] font-medium text-pink-400/50 flex items-center gap-1 hidden sm:flex">
      ...
      {foodRedirectToast && (
        <div style={{
          ...
          background: isCreatorMode ? 'linear-gradient(135deg,#16a34a,#15803d)' : '#1e293b',
          border:`1px solid ${t.glow}50`,
          borderRadius:16, padding:'10px 18px',
          boxShadow:`0 8px 30px rgba(0,0,0,0.4), 0 0 20px ${t.glow}20`,
          ...
          <div>
            <p style={{fontSize:12,fontWeight:800,color:'#fff',lineHeight:1.2,margin:0}}>Log that in Health Pulse!</p>
            <p style={{fontSize:10,color:'rgba(255,255,255,0.65)',lineHeight:1.3,margin:'2px 0 0'}}>Tap the 🥗 button → track meals &amp; calories accurately</p>
          </div>
        </div>
      )}
      ...
      {/* ══════════════════════════════════════════════════════════════════
          QUIZ GENERATOR — subject / chapter / class / level / exam / timer
      ══════════════════════════════════════════════════════════════════ */}
      {showQuizGen && (
        <div className="fixed inset-0 z-[110] flex items-end md:items-center justify-center p-3 md:p-6"
          style={{backdropFilter:'blur(16px)',background:t.isLight?'rgba(0,0,0,0.35)':'rgba(0,0,0,0.65)'}}>
          <div className="w-full max-w-md rounded-3xl overflow-hidden"
            style={{
              background: t.isLight
                ? 'linear-gradient(160deg,#fdf9f3 0%,#fef6ee 100%)'
                : `linear-gradient(160deg,#0c0f1e 0%,#0f1325 100%)`,
              border: `1.5px solid ${t.isLight?'rgba(180,120,60,0.18)':t.glow+'40'}`,
              boxShadow: t.isLight
                ? '0 32px 80px rgba(0,0,0,0.18), inset 0 1px 0 rgba(255,255,255,0.9)'
                : `0 32px 80px rgba(0,0,0,0.55), 0 0 0 1px ${t.glow}12, inset 0 1px 0 rgba(255,255,255,0.04)`,
            }}>
            <div className="flex items-center justify-between px-5 py-4"
              style={{
                borderBottom: `1px solid ${t.isLight?'rgba(0,0,0,0.07)':t.glow+'18'}`,
                background: t.isLight
                  ? 'linear-gradient(135deg,rgba(255,255,255,0.8),rgba(253,243,230,0.6))'
                  : `linear-gradient(135deg,${t.glow}12,transparent)`,
              }}>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-2xl flex items-center justify-center text-base"
                  style={{background:`linear-gradient(135deg,${t.glow}30,${t.glow}15)`,border:`1px solid ${t.glow}40`}}>🧠</div>
                <div>
                  <p className="font-black text-[14px] leading-none" style={{color:t.isLight?'#1e1a12':t.glow}}>Quiz Generator</p>
                  <p className="text-[10px] mt-0.5" style={{color:t.isLight?'#9a7a4a':'rgba(255,255,255,0.35)'}}>AI-powered practice tests</p>
                </div>
              </div>
              <button onClick={()=>setShowQuizGen(false)}
                className="w-8 h-8 rounded-xl flex items-center justify-center transition-all hover:scale-110"
                style={{background:t.isLight?'rgba(0,0,0,0.06)':'rgba(255,255,255,0.07)',border:`1px solid ${t.isLight?'rgba(0,0,0,0.08)':'rgba(255,255,255,0.08)'}`}}>
                <X size={13} style={{color:t.isLight?'#64748b':'#94a3b8'}}/>
              </button>
            </div>
            <QuizGeneratorPanel glow={t.glow} isLight={t.isLight}
              onGenerate={(prompt)=>{
      ...
      {/* ══════════════════════════════════════════════════════════════════
          MEMORY TIMELINE — visual timeline of what Tessa remembers
      ══════════════════════════════════════════════════════════════════ */}
      {showMemTimeline && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4"
          style={{backdropFilter:'blur(16px)',background:t.isLight?'rgba(0,0,0,0.35)':'rgba(0,0,0,0.65)'}}>
          <div className="w-full max-w-sm rounded-3xl overflow-hidden max-h-[82vh] flex flex-col"
            style={{
              background: t.isLight?'linear-gradient(160deg,#fdf9f3,#fef6ee)':' linear-gradient(160deg,#0c0f1e,#0f1325)',
              border:`1.5px solid ${t.isLight?'rgba(180,120,60,0.18)':t.glow+'40'}`,
              boxShadow: t.isLight?'0 32px 80px rgba(0,0,0,0.18),inset 0 1px 0 rgba(255,255,255,0.9)':`0 32px 80px rgba(0,0,0,0.55),0 0 0 1px ${t.glow}12,inset 0 1px 0 rgba(255,255,255,0.04)`,
            }}>
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 flex-shrink-0"
              style={{borderBottom:`1px solid ${t.isLight?'rgba(0,0,0,0.07)':t.glow+'18'}`,background:t.isLight?'linear-gradient(135deg,rgba(255,255,255,0.8),rgba(253,243,230,0.6))':`linear-gradient(135deg,${t.glow}12,transparent)`}}>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-2xl flex items-center justify-center"
                  style={{background:`linear-gradient(135deg,${t.glow}30,${t.glow}15)`,border:`1px solid ${t.glow}40`}}>
                  <span style={{fontSize:15}}>🧬</span></div>
                <div>
                  <p className="font-black text-[14px] leading-none" style={{color:t.isLight?'#1e1a12':t.glow}}>Memory Timeline</p>
                  <p className="text-[10px] mt-0.5" style={{color:t.isLight?'#9a7a4a':'rgba(255,255,255,0.35)'}}>What Tessa remembers about you</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={()=>{if(confirm('Clear all memories?'))clearAllMemories();}}
                  className="text-[9px] px-2.5 py-1.5 rounded-lg font-semibold transition-all"
                  style={{background:'rgba(239,68,68,0.10)',border:'1px solid rgba(239,68,68,0.25)',color:'#f87171'}}>Clear</button>
                <button onClick={()=>setShowMemTimeline(false)}
                  className="w-8 h-8 rounded-xl flex items-center justify-center transition-all hover:scale-110"
                  style={{background:t.isLight?'rgba(0,0,0,0.06)':'rgba(255,255,255,0.07)',border:`1px solid ${t.isLight?'rgba(0,0,0,0.08)':'rgba(255,255,255,0.08)'}`}}>
                  <X size={13} style={{color:t.isLight?'#64748b':'#94a3b8'}}/>
                </button>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2" style={{scrollbarWidth:'thin'}}>
              {getAllMemories().length === 0 ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-[12px] mt-3 leading-relaxed" style={{color:t.isLight?'#9a7a4a':'rgba(255,255,255,0.35)'}}>No memories yet. Chat with Tessa and she'll start remembering things about you.</p>
                </div>
              ) : getAllMemories().map((mem:any, i:number) => {
                const text = typeof mem==='string'?mem:mem.content||JSON.stringify(mem);
                const date = mem.timestamp ? new Date(mem.timestamp).toLocaleDateString('en-IN',{day:'numeric',month:'short'}) : null;
                return (
                  <div key={i} className="flex gap-3 items-start">
                    <div className="flex flex-col items-center flex-shrink-0" style={{width:18}}>
                      <div className="w-2 h-2 rounded-full mt-1.5 flex-shrink-0" style={{background:t.glow,boxShadow:`0 0 8px ${t.glow}70`}}/>
                      {i < getAllMemories().length-1 && <div className="w-px flex-1 mt-1" style={{background:`${t.glow}22`,minHeight:18}}/>}
                    </div>
                    <div className="flex-1 pb-2.5 pl-1">
                      <p className="text-[11.5px] leading-relaxed font-medium" style={{color:t.isLight?'#3d2c14':'rgba(255,255,255,0.82)'}}>{text}</p>
                      {date && <p className="text-[9px] mt-0.5 font-medium" style={{color:t.isLight?'#b08050':t.glow+'99'}}>{date}</p>}
                    </div>
                  </div>
                );
              })}
      ...
      {/* ══════════════════════════════════════════════════════════════════
          TESSA PERSONAS — switch between companion modes
      ══════════════════════════════════════════════════════════════════ */}
      {/* Persona chip in header toolbar (desktop only) — added inline below */}
      ...
      {showProfiles && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-xs rounded-3xl overflow-hidden shadow-2xl"
            style={{background:t.isLight?'#fdf8f0':'#0c0f1e',border:`1.5px solid ${t.isLight?'rgba(0,0,0,0.10)':t.glow+'35'}`,boxShadow:`0 24px 64px rgba(0,0,0,0.40), 0 0 0 1px ${t.isLight?'rgba(0,0,0,0.05)':t.glow+'18'}`}}>
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{borderColor:t.isLight?'rgba(0,0,0,0.07)':t.glow+'14'}}>
              <div className="flex items-center gap-2">
                <span style={{fontSize:18}}>👤</span>
                <span className="font-black text-[14px]" style={{color:t.glow}}>Profiles</span>
              </div>
              <button onClick={()=>setShowProfiles(false)} className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.btnS}`}><X size={14}/></button>
            </div>
            <div className="px-4 py-4 space-y-2">
              {/* Built-in profiles */}
              {[
                {id:'standard', name:'Standard', emoji:'💬', desc:'General purpose', mode:'standard' as const},
                {id:'creator',  name:'Ankit (Creator)', emoji:'💝', desc:'Personal AI mode', mode:'creator' as const},
                {id:'study',    name:'Study Mode', emoji:'📚', desc:'Focused on academics', mode:'standard' as const},
                {id:'creative', name:'Creative Mode', emoji:'🎨', desc:'Writing & brainstorming', mode:'standard' as const},
              ].map(p=>(
                <button key={p.id}
                  onClick={()=>{
                    if(p.mode==='creator' && !isCreatorMode){alert('Enter Creator Mode first.');}
                    else {
                      setTessaPersona(p.id==='study'?'studybuddy':p.id==='creative'?'mentor':'companion');
                      setShowProfiles(false);
                    }
                  }}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl transition-all active:scale-98 text-left"
                  style={{
                    background: tessaPersona===(p.id==='study'?'studybuddy':p.id==='creative'?'mentor':'companion') ? `${t.glow}12` : t.isLight?'#f8fafc':'#1c2333',
                    border: `1px solid ${t.glow}20`,
                  }}>
                  <span style={{fontSize:22}}>{p.emoji}</span>
                  <div>
                    <p className="text-[12px] font-bold" style={{color:t.isLight?'#1e293b':'#fff'}}>{p.name}</p>
                    <p className={`text-[10px] ${t.sub}`}>{p.desc}</p>
                  </div>
                </button>
              ))}
              <div className="pt-1">
                <p className={`text-[10px] text-center ${t.sub}`}>Active persona: <strong style={{color:t.glow}}>{tessaPersona}</strong></p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ══ SHORT NOTES — Study Buddy / Mentor only ══════════════════════════ */}
      {showShortNotes && (
        <div className="fixed inset-0 z-[115] flex items-center justify-center p-4"
          style={{backdropFilter:'blur(16px)',background:'rgba(0,0,0,0.65)'}}
          onClick={e=>{if(e.target===e.currentTarget)setShowShortNotes(false);}}>
          <div className="w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl max-h-[88vh] flex flex-col"
            style={{
              background:t.isLight
                ?'linear-gradient(160deg,#fffdf5 0%,#fef9e7 100%)'
                :'linear-gradient(160deg,#0d1021 0%,#080c18 100%)',
              border:`1.5px solid ${t.isLight?'rgba(196,160,50,0.3)':t.glow+'38'}`,
              boxShadow:`0 32px 80px rgba(0,0,0,0.45), inset 0 1px 0 ${t.isLight?'rgba(255,255,255,0.95)':'rgba(255,255,255,0.07)'}`,
            }}>
            <div className="flex items-center justify-between px-5 py-4 flex-shrink-0"
              style={{borderBottom:`1px solid ${t.isLight?'rgba(196,160,50,0.18)':t.glow+'15'}`}}>
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-2xl flex items-center justify-center"
                  style={{background:`linear-gradient(135deg,${t.glow}22,${t.glow}0e)`,border:`1.5px solid ${t.glow}40`}}>
                  <span style={{fontSize:18}}>📒</span>
                </div>
                <div>
                  <p className="font-black text-[14px]" style={{color:t.glow}}>Short Notes</p>
                  <p className="text-[9px]" style={{color:t.isLight?'#92806a':'rgba(255,255,255,0.35)'}}>
                    {tessaPersona==='studybuddy'||tessaPersona==='mentor'?`${shortNotes.length} notes saved`:'Study persona required'}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {shortNotes.length>0&&(
                  <button onClick={()=>{setShortNotes([]);localStorage.removeItem('tessa-shortnotes');}}
                ...
                <button onClick={()=>setShowShortNotes(false)}
                  className="w-8 h-8 rounded-xl flex items-center justify-center transition-colors hover:bg-white/10"
                  style={{border:`1px solid ${t.glow}22`}}>
                  <X size={13} style={{color:t.glow}}/>
                </button>
              </div>
            </div>
            {tessaPersona!=='studybuddy'&&tessaPersona!=='mentor'?(
              <div className="flex flex-col items-center py-12 px-6 text-center">
                <div className="w-16 h-16 rounded-3xl flex items-center justify-center mb-4"
                  style={{background:`${t.glow}10`,border:`1.5px solid ${t.glow}22`}}>
                  <span style={{fontSize:30}}>🔒</span>
                </div>
                <p className="font-black text-[14px] mb-2" style={{color:t.glow}}>Study Mode Required</p>
                <p className="text-[11px] leading-relaxed max-w-[210px]" style={{color:t.isLight?'#78716c':'rgba(255,255,255,0.45)'}}>
                  Short Notes uses AI to extract key concepts. Available in <strong style={{color:t.glow}}>Study Buddy</strong> or <strong style={{color:t.glow}}>Mentor</strong> persona.
                </p>
              </div>
            ):shortNotes.length===0?(
              <div className="flex flex-col items-center py-12 px-6 text-center">
                ...
                <p className="font-black text-[14px] mb-2" style={{color:t.glow}}>Ready to take notes</p>
                <p className="text-[11px] leading-relaxed max-w-[210px]" style={{color:t.isLight?'#78716c':'rgba(255,255,255,0.45)'}}>
                  As you study with Tessa, she'll auto-save key formulas, definitions, and concepts here.
                </p>
              </div>
            ):(
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2" style={{scrollbarWidth:'thin'}}>
                {shortNotes.map((note:any,i:number)=>(
                  <div key={i} className="rounded-2xl p-3.5"
                    style={{
                      background:t.isLight?'rgba(255,253,235,0.9)':'rgba(255,255,255,0.035)',
                      border:`1px solid ${t.isLight?'rgba(196,160,50,0.2)':t.glow+'18'}`,
                    }}>
                    <p className="text-[9px] font-black uppercase tracking-[0.1em] mb-1.5" style={{color:t.glow}}>{note.topic}</p>
                    <p className="text-[12px] leading-relaxed font-mono" style={{color:t.isLight?'#374151':'#d1d5db'}}>{note.formula}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {showTimeCapsule && (
        <div className="fixed inset-0 z-[115] flex items-center justify-center p-4"
          style={{backdropFilter:'blur(18px) saturate(1.4)',background:'rgba(0,0,0,0.62)'}}>
          <TimeCapsulePanel
            glow={t.glow} isLight={t.isLight}
            capsules={capsules}
            onSave={(cap)=>{
              const updated=[...capsules,cap];
              setCapsules(updated);
              try{localStorage.setItem('tessa-capsules',JSON.stringify(updated));}catch{}
      ...
      {showWorldClock && (
        <div className="fixed inset-0 z-[115] flex items-center justify-center p-4"
          style={{backdropFilter:'blur(18px) saturate(1.4)',background:'rgba(0,0,0,0.62)'}}
          onClick={e=>{if(e.target===e.currentTarget)setShowWorldClock(false);}}>
          <WorldClockPanel glow={t.glow} isLight={t.isLight} onClose={()=>setShowWorldClock(false)}/>
        </div>
      )}
      ...
      {showChangelog && (
        <div className="fixed inset-0 z-[115] flex items-center justify-center p-4"
          style={{backdropFilter:'blur(18px) saturate(1.4)',background:'rgba(0,0,0,0.62)'}}>
          <ChangelogPanel glow={t.glow} isLight={t.isLight} onClose={()=>setShowChangelog(false)}/>
        </div>
      )}
