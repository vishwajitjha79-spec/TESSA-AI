...
export default function Home() {
  ...
  // Formula sheet
  const [formulaSheet,    setFormulaSheet]   = useState<{topic:string;formula:string}[]>(
    () => { try { return JSON.parse(localStorage.getItem('tessa-formulas')||'[]'); } catch { return []; } }
  );
    ...
    return (
      ...
      >
        <div style={{
          ...
          maxWidth: callFullscreen ? 480 : undefined,
...
`}</style>
              ...
              <div className="space-y-2.5 pb-2">
                ...
                {messages.map((msg,idx)=>{
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`msg-row group flex gap-2.5 ${isUser?'justify-end':'justify-start'} ${isGrouped?'mt-0.5':'mt-2.5'} ${isUser?'pl-8 md:pl-16':'pr-8 md:pr-16'}`}
                    >
        ...
        {!showDashboard&&(
          <div className={`flex-shrink-0 ${t.bar} px-3 md:px-6 py-3 safe-bottom`}>
            <div className="max-w-2xl mx-auto w-full">
              ...
              {/* ── SMART FOLLOW-UPS ── show suggested questions after Tessa replies */}
              {followUps.length > 0 && !isLoading && (
                <div className="mb-2 flex flex-wrap gap-1.5 animate-in fade-in slide-in-from-bottom-2">
                  <span className="text-[9px] font-semibold w-full mb-0.5" style={{color:t.glow,letterSpacing:'0.06em'}}>
                    ✦ FOLLOW UP
                  </span>
                  {followUps.map((q,i)=>(
                    <button key={i}
                      onClick={()=>{setInput(q);setFollowUps([]);setTimeout(()=>textareaRef.current?.focus(),50);}}
                  ...
                  <button onClick={()=>setFollowUps([])}
                    className="text-[9px] px-2 py-1 rounded-full opacity-40 hover:opacity-70 transition-opacity"
                    style={{color:t.glow}}>✕</button>
                </div>
              )}
              ...
              <div className="flex items-center justify-between mt-1.5 px-0.5 gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {/* ── Feature toolbar v8 ── */}
                  {[
                    {ico:'🧠', lbl:'Quiz',    fn:()=>setShowQuizGen(true),    title:'Quiz Generator'},
                    {ico:'🧬', lbl:'Memory',  fn:()=>setShowMemTimeline(true),title:'Memory Timeline'},
                    {ico:'✦',  lbl:tessaPersona!=='companion'?tessaPersona:'Persona',
                      fn:()=>setShowProfiles(true), title:'Personas',
                      active:tessaPersona!=='companion'},
                    {ico:'📐', lbl:'Formulas',fn:()=>setShowFormulaSheet(true),title:'Formula Sheet',
                      badge:formulaSheet.length>0?String(formulaSheet.length):undefined},
                    {ico:'⚡', lbl:'Code',    fn:()=>setShowCodeSandbox(true),title:'Code Sandbox'},
                    {ico:'📬', lbl:'Capsule', fn:()=>setShowTimeCapsule(true),title:'Time Capsule'},
                    {ico:'🏆', lbl:'Wins',    fn:()=>setShowAchievements(true),title:'Achievements',
                      badge:Object.values(achievements).filter(Boolean).length>0?String(Object.values(achievements).filter(Boolean).length):undefined},
                    {ico:'🪞', lbl:'Report',  fn:()=>setShowSelfReport(true), title:"Tessa's Self-Report"},
                    {ico:'📰', lbl:'News',    fn:()=>setShowNewsTicker(p=>!p),title:'Live News Ticker',
                      active:showNewsTicker},
                  ].map(btn=>(
                    <button key={btn.lbl} onClick={btn.fn} title={btn.title}
                      className="relative flex items-center gap-1 px-2 py-1 rounded-lg transition-all active:scale-95"
                      style={{
                        background:btn.active?`${t.glow}18`:`${t.glow}0a`,
                        border:`1px solid ${t.glow}${btn.active?'40':'18'}`,
                      }}>
                      <span style={{fontSize:11}}>{btn.ico}</span>
                      <span className={`text-[9px] font-semibold hidden md:block`}
                        style={{color:btn.active?t.glow:undefined}}>{btn.lbl}</span>
                      {btn.badge && (
                        <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full flex items-center justify-center text-[7px] font-black text-white"
                          style={{background:t.glow,fontSize:'7px'}}>{btn.badge}</span>
                      )}
                    </button>
                  ))}
      ...
      {showQuizGen && (
        <div className="fixed inset-0 z-[110] flex items-end md:items-center justify-center p-4"
          ...
          <div className="w-full max-w-md rounded-3xl overflow-hidden shadow-2xl"
            style={{background:t.isLight?'#fff':'#0f1117',border:`1px solid ${t.glow}30`}}>
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{borderColor:`${t.glow}18`}}>
              <div className="flex items-center gap-2">
                <span style={{fontSize:20}}>🧠</span>
                <span className="font-black text-[15px]" style={{color:t.glow}}>Quiz Generator</span>
              </div>
              <button onClick={()=>setShowQuizGen(false)} className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.btnS}`}><X size={14}/></button>
            </div>
      ...
      {showMemTimeline && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4"
          style={{backdropFilter:'blur(12px)',background:'rgba(0,0,0,0.55)'}}>
          <div className="w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl max-h-[80vh] flex flex-col"
            style={{background:t.isLight?'#fff':'#0f1117',border:`1px solid ${t.glow}30`}}>
            <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0" style={{borderColor:`${t.glow}18`}}>
              <div className="flex items-center gap-2">
                <span style={{fontSize:18}}>🧬</span>
                <span className="font-black text-[14px]" style={{color:t.glow}}>Memory Timeline</span>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={()=>{if(confirm('Clear all memories?'))clearAllMemories();}}
                  className="text-[10px] px-2 py-1 rounded-lg text-red-400" style={{border:'1px solid #ef444430'}}>Clear all</button>
                <button onClick={()=>setShowMemTimeline(false)} className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.btnS}`}><X size={14}/></button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2" style={{scrollbarWidth:'thin'}}>
              {getAllMemories().length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className={`text-[12px] mt-2 ${t.sub}`}>No memories yet. Chat with Tessa and she'll start remembering things about you.</p>
                </div>
              ) : (
                  ...
                  return (
                    <div key={i} className="flex gap-3 items-start">
                      <div className="flex flex-col items-center" style={{width:20,flexShrink:0}}>
                        <div className="w-2.5 h-2.5 rounded-full mt-1" style={{background:t.glow,boxShadow:`0 0 6px ${t.glow}60`}}/>
                        {i < getAllMemories().length-1 && <div className="w-px flex-1 mt-1" style={{background:`${t.glow}20`,minHeight:16}}/>}
                      </div>
                      <div className="flex-1 pb-2">
                        <p className={`text-[11px] leading-relaxed ${t.isLight?'text-slate-700':'text-white/80'}`}>{text}</p>
                        {date && <p className={`text-[9px] mt-0.5 ${t.sub}`}>{date}</p>}
                      </div>
                    </div>
                  );
                })
      ...
      {showAchievements && (
        <div className="fixed inset-0 z-[115] flex items-center justify-center p-4"
          style={{backdropFilter:'blur(14px)',background:'rgba(0,0,0,0.6)'}}>
          <div className="w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl max-h-[85vh] flex flex-col"
            style={{background:t.isLight?'#fff':'#0c0f1a',border:`1px solid ${t.glow}30`}}>
            <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0" style={{borderColor:`${t.glow}18`}}>
              <div className="flex items-center gap-2"><span style={{fontSize:18}}>🏆</span>
                <span className="font-black text-[14px]" style={{color:t.glow}}>Achievements</span></div>
              <button onClick={()=>setShowAchievements(false)} className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.btnS}`}><X size={14}/></button>
            </div>
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5" style={{scrollbarWidth:'thin'}}>
              {/* Study heatmap */}
              <div>
                <p className="text-[10px] font-black mb-2" style={{color:t.glow}}>ACTIVITY HEATMAP</p>
                <StudyHeatmap log={studyLog} glow={t.glow} isLight={t.isLight}/>
              </div>
              <div className="border-t pt-3" style={{borderColor:`${t.glow}18`}}>
                <p className="text-[10px] font-black mb-2" style={{color:t.glow}}>BADGES</p>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    {id:'first_ten',  emoji:'🌱', title:'First Steps',   desc:'10 messages sent'},
                    {id:'century',    emoji:'💯', title:'Century',       desc:'100 messages sent'},
                    {id:'night_owl',  emoji:'🦉', title:'Night Owl',     desc:'Chatted after midnight'},
                    {id:'early_bird', emoji:'🌅', title:'Early Bird',    desc:'Chatted before 7am'},
                    {id:'web_explorer',emoji:'🌐',title:'Web Explorer',  desc:'Used web search'},
                    {id:'creator_mode',emoji:'💝',title:'Creator',       desc:'Unlocked Creator Mode'},
                  ].map(a=>(
                    <div key={a.id} className="flex items-center gap-2 p-2 rounded-xl"
                      style={{
                        background: achievements[a.id]?`${t.glow}12`:t.isLight?'#f8fafc':'#131825',
                        border:`1px solid ${achievements[a.id]?t.glow+'35':t.glow+'10'}`,
                        opacity: achievements[a.id] ? 1 : 0.4,
                      }}>
                      <span style={{fontSize:22,filter:achievements[a.id]?'none':'grayscale(1)'}}>{a.emoji}</span>
                      <div><p className="text-[10px] font-bold" style={{color:achievements[a.id]?t.glow:undefined}}>{a.title}</p>
                        <p className={`text-[8px] ${t.sub}`}>{a.desc}</p></div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {showFormulaSheet && (
        <div className="fixed inset-0 z-[115] flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl max-h-[85vh] flex flex-col"
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0" style={{borderColor:`${t.glow}18`}}>
              <div className="flex items-center gap-2"><span style={{fontSize:18}}>📐</span>
                <span className="font-black text-[14px]" style={{color:t.glow}}>Formula Sheet</span></div>
              <div className="flex items-center gap-2">
                <button onClick={()=>{setFormulaSheet([]);localStorage.removeItem('tessa-formulas');}}
                  className="text-[9px] px-2 py-1 rounded-lg text-red-400 border border-red-400/30">Clear</button>
                <button onClick={()=>setShowFormulaSheet(false)} className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.btnS}`}><X size={14}/></button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto" style={{scrollbarWidth:'thin'}}>
              {formulaSheet.length===0 ? (
                <div className="text-center py-10">
                  ...
                  <p className={`text-[12px] mt-2 ${t.sub}`}>Formulas are auto-collected from Tessa's replies. Ask a maths or science question!</p>
                </div>
              ) : (
                <div className="divide-y" style={{borderColor:`${t.glow}10`}}>
                  {formulaSheet.map((f,i)=>(
                    <div key={i} className="px-4 py-2.5">
                      <p className="text-[9px] font-semibold mb-0.5 uppercase tracking-wide" style={{color:t.glow}}>{f.topic}</p>
                      <code className="text-[12px] font-mono" style={{color:t.isLight?'#1e293b':'#e2e8f0'}}>{f.formula}</code>
                    </div>
                  ))}
                </div>
              )}
      ...
      {showCodeSandbox && (
        <div className="fixed inset-0 z-[115] flex items-center justify-center p-4"
          ...
          <CodeSandboxPanel
            glow={t.glow} isLight={t.isLight}
            code={sandboxCode} setCode={setSandboxCode}
            output={sandboxOutput} setOutput={setSandboxOutput}
            onClose={()=>setShowCodeSandbox(false)}
            onSendToTessa={(code)=>{
              setShowCodeSandbox(false);
              setInput(`Review this code:\n\`\`\`\n${code}\n\`\`\``);
              setTimeout(()=>textareaRef.current?.focus(),100);
            }}/>
        </div>
      )}

      {/* ══ TESSA SELF-REPORT PANEL ════════════════════════════════════════ */}
      {showSelfReport && (
        <div className="fixed inset-0 z-[115] flex items-center justify-center p-4"
          style={{backdropFilter:'blur(14px)',background:'rgba(0,0,0,0.6)'}}>
          <SelfReportPanel
            glow={t.glow} isLight={t.isLight}
            report={selfReport} setReport={setSelfReport}
