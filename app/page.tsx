...
Style: Direct, warm, specific. No generic advice. Use actual numbers from his data. Max 200 words total. No fluff.`;
  ...
  const sendMessage = async (override?: string) => {
    ...
    try {
      ...
      // Generate smart follow-ups for this reply (non-blocking)
      setFollowUps([]);
            ...
            <div className="max-w-2xl mx-auto w-full">
              ...
              <div className="flex items-center justify-between mt-1.5 px-0.5 gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {/* ── Feature buttons ── */}
                  <button onClick={()=>setShowQuizGen(true)} title="Quiz Generator"
                    className="flex items-center gap-1 px-2 py-1 rounded-lg transition-all active:scale-95"
                    style={{background:`${t.glow}0a`,border:`1px solid ${t.glow}20`}}>
                    <span style={{fontSize:11}}>🧠</span>
                    <span className={`text-[9px] font-semibold hidden sm:block ${t.sub}`}>Quiz</span>
                  </button>
                  <button onClick={()=>setShowMemTimeline(true)} title="Memory Timeline"
                    className="flex items-center gap-1 px-2 py-1 rounded-lg transition-all active:scale-95"
                    style={{background:`${t.glow}0a`,border:`1px solid ${t.glow}20`}}>
                    <span style={{fontSize:11}}>🧬</span>
                    <span className={`text-[9px] font-semibold hidden sm:block ${t.sub}`}>Memory</span>
                  </button>
                  <button onClick={()=>setShowProfiles(true)} title="Switch Persona / Profile"
                    className="flex items-center gap-1 px-2 py-1 rounded-lg transition-all active:scale-95"
                    style={{background:tessaPersona!=='companion'?`${t.glow}15`:`${t.glow}0a`,border:`1px solid ${t.glow}${tessaPersona!=='companion'?'40':'20'}`}}>
                    <span style={{fontSize:11}}>✦</span>
                    <span className={`text-[9px] font-semibold hidden sm:block`}
                      style={{color:tessaPersona!=='companion'?t.glow:undefined}}>{tessaPersona!=='companion'?tessaPersona:'Persona'}</span>
                  </button>
