...
Style: Direct, warm, specific. No generic advice. Use actual numbers from his data. Max 200 words total. No fluff.`;
  ...
  // Interrupt ref for barge-in detection
  ...
  const callSpeakTessa = (text: string, onDone: () => void) => {
    ...
    const clean = text
      .replace(/[*_~`#]/g, '').replace(/TESSA/gi, '').trim().slice(0, 300);
    const lang = language === 'hi' ? 'hi-IN' : 'en-US';
    const u = new SpeechSynthesisUtterance(clean);
    u.pitch = 1.75; u.rate = 1.22; u.volume = 0.93; u.lang = lang;
    const trySpeak = () => {
      const v = getBestVoice(lang); if (v) u.voice = v;
      const keepAlive = setInterval(() => {
        try { if (window.speechSynthesis.speaking && window.speechSynthesis.paused) window.speechSynthesis.resume(); } catch {}
      }, 200);
      // Barge-in detection via AudioContext energy
      let iCtx: AudioContext|null = null;
      if (typeof AudioContext !== 'undefined' && navigator.mediaDevices?.getUserMedia) {
        navigator.mediaDevices.getUserMedia({audio:true}).then(stream => {
          try {
            iCtx = new AudioContext();
            const ana = iCtx.createAnalyser();
            ana.fftSize = 256;
            iCtx.createMediaStreamSource(stream).connect(ana);
            ...
            interruptCheckRef.current = setInterval(() => {
              if (!isSpeakingRef.current || triggered) { clearInterval(interruptCheckRef.current!); return; }
              ...
              if (avg > 24) {
                ...
                try { iCtx?.close(); } catch {}
                isSpeakingRef.current = false;
                setTimeout(startCallListening, 120);
              }
            }, 90);
      ...
      u.onend   = () => { cleanup(); onDone(); };
      u.onerror = () => { cleanup(); onDone(); };
      window.speechSynthesis.speak(u);
    };
  ...
  const startCallListening = () => {
    ...
    if (!SR) return;
    const r = new SR();
    // Use broader language matching: en-US picks up Indian English better on some devices
    r.lang = language === 'hi' ? 'hi-IN' : language === 'hinglish' ? 'hi-IN' : 'en-US';
    r.continuous = false;       // false = auto-detects end of speech (better than true on mobile)
    r.interimResults = true;    // show live text while user is speaking
    r.maxAlternatives = 3;      // get top 3 candidates, pick highest confidence
    let final = '';
    ...
    r.onresult = (e: any) => {
      ...
      for (let i = e.resultIndex; i < e.results.length; i++) {
        if (e.results[i].isFinal) {
          ...
          if (best.confidence > bestConfidence || final === '') {
      ...
      setInterimText(interim || final.trim());
    };
    ...
    r.onend = async () => {
      ...
      // Ignore very short/noisy captures (< 2 chars)
      if (!said || said.length < 2) { setTimeout(startCallListening, 120); return; }
      setCallTranscript(p => [...p, { role: 'user', text: said }]);
      setCallStatus('thinking');
      callMessagesRef.current.push({ role: 'user', content: said });
      if (callMessagesRef.current.length > 20) callMessagesRef.current = callMessagesRef.current.slice(-20);
      try {
        const res = await fetch('/api/chat', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            ...
            maxTokens: 80, // short = fast = natural voice flow
            _systemOverride: `Voice call. ONE sentence reply only. No punctuation issues. No markdown. Warm, direct, human.${isCreatorMode ? " Personal AI for Ankit." : ''}`,
          }),
        });
        const data = await res.json();
        const reply = (data.content || 'Hmm, say that again?')
          .replace(/\*\*/g,'').replace(/\*/g,'').replace(/#{1,6}\s/g,'').replace(/[_~`]/g,'')
          .replace(/\n+/g,' ').trim().slice(0, 320);
        callMessagesRef.current.push({ role: 'assistant', content: reply });
        setCallTranscript(p => [...p, { role: 'tessa', text: reply }]);
        callSpeakTessa(reply, () => {
          if (callLoopRef.current) setTimeout(startCallListening, 80);
        });
      } catch {
        callSpeakTessa('Lost connection for a moment!', () => {
    ...
    r.onerror = (e: any) => {
      ...
      // 'no-speech' and 'network' are transient — just retry
      if (callLoopRef.current) setTimeout(startCallListening, 180);
    };
    recognitionRef.current = r;
    try { r.start(); } catch { if (callLoopRef.current) setTimeout(startCallListening, 300); }
  };
  ...
  // ── Median/GoNative permission bridge ────────────────────────────────────
  // Median wraps in a native WebView. Standard getUserMedia alone does NOT
  // trigger the OS permission dialog — you must call the GoNative JS bridge
  // first to request the native RECORD_AUDIO / NSMicrophoneUsageDescription.
  const requestMedianMicPermission = (): Promise<void> => {
    return new Promise((resolve, reject) => {
      const gn = (window as any).gonative;
      if (gn?.permissions?.requestMicrophonePermission) {
        // GoNative / Median JS bridge — triggers the native OS dialog
        gn.permissions.requestMicrophonePermission({
          callback: (result: any) => {
            if (result?.status === 'authorized' || result?.granted === true) {
              resolve();
            } else {
              reject(new Error('median_denied'));
            }
          }
        });
      } else if (gn?.statusbar) {
        // Older GoNative SDK — try gonative.audio.start as permission probe
        resolve(); // Let getUserMedia handle it
      } else {
        // Not in Median app — go straight to getUserMedia
        resolve();
      }
    });
  };
  ...
  const startVoiceCall = () => {
    ...
    const isMedianApp = typeof (window as any).gonative !== 'undefined'
      || /gonative|GoNative/i.test(navigator.userAgent || '');

    const doStartCall = () => {
      // Now actually request mic audio stream
      const micPromise: Promise<any> = navigator.mediaDevices?.getUserMedia({ audio: true })
        ?? Promise.reject(new Error('MediaDevices not available'));

      micPromise.then(() => {
        playRingTone();
        setVoiceCallOpen(true);
        // Auto-fullscreen on mobile (screen width < 640px)
        ...
        const creatorGreets = [
          hour < 6  ? "It's really late Ankit… is everything okay?" :
          hour < 12 ? "Good morning! Ready to take on the day?" :
          hour < 17 ? "Hey! What are we working on?" :
          hour < 21 ? "Hey Ankit, good evening! What's on your mind?" :
                      "Getting late… but I'm here. What's up?",
          "Hey! Missed your voice. What do you need?",
          "Hi! I'm all ears — what's going on?",
          "Hey Ankit! Let's talk. What's happening?",
          "Oh, a voice call! Love these. What's up?",
        ];
        const standardGreets = [
          "Hi! I'm Tessa. How can I help?",
          "Hello! What would you like to talk about?",
          "Hey there! What's on your mind?",
        ];
        const greetPool = isCreatorMode ? creatorGreets : standardGreets;
        ...
        if (isMedianApp) {
          ...
          alert('Mic access denied. Click the 🔒 lock icon in your browser address bar → Microphone → Allow.');
        }
      });
    };

    if (isMedianApp) {
      // Step 1: request native permission via GoNative bridge, then start
      requestMedianMicPermission()
        .then(doStartCall)
        .catch((err: any) => {
          if (err?.message === 'median_denied') {
            alert('Microphone permission denied.\n\nTo fix:\n1. Open your phone Settings\n2. Find TESSA app\n3. Tap Permissions → Microphone → Allow\n4. Come back and try again.');
          } else {
            // Bridge not available on this SDK version — fall through to getUserMedia
            doStartCall();
          }
        });
    } else {
  ...
  // ── Voice Call Window v2 — fullscreen-first, cinematic design ─────────────
  const VoiceCallWindow = () => {
    if (!voiceCallOpen) return null;

    const onPtrDown = (e: React.PointerEvent<HTMLDivElement>) => {
      if (callFullscreen || (e.target as HTMLElement).closest('button')) return;
    ...
    const onPtrMove = (e: React.PointerEvent<HTMLDivElement>) => {
      if (!callDragging.current || callFullscreen) return;
      const nx = Math.max(8, Math.min(window.innerWidth - 308, callDragStart.current.px + e.clientX - callDragStart.current.mx));
      const ny = Math.max(8, Math.min(window.innerHeight - 420, callDragStart.current.py + e.clientY - callDragStart.current.my));
      (e.currentTarget as HTMLDivElement).style.transform = `translate(${nx}px, ${ny}px)`;
    };
    ...
    const px = callPos?.x ?? (typeof window !== 'undefined' ? Math.max(8, window.innerWidth / 2 - 150) : 100);
    const py = callPos?.y ?? 60;
    const fmt = (s: number) => `${Math.floor(s/60)}:${String(s%60).padStart(2,'0')}`;

    const sc = callStatus==='listening'?'#22c55e':callStatus==='thinking'?'#a78bfa':callStatus==='speaking'?'#f59e0b':'#475569';
    ...
    // 36-bar waveform heights — staggered pattern
    const barPattern = [3,5,8,12,18,24,20,14,9,6,4,7,13,22,28,32,28,22,14,8,5,7,15,24,20,13,8,5,4,6,11,18,14,9,5,3];
    ...
    return (
      ...
      {callFullscreen && (
        <div style={{
          ...
          background:'linear-gradient(175deg,#050412 0%,#0c0824 40%,#060e22 100%)',
        }}>
          {/* Background radial glow — NO filter:blur (GPU killer), pure gradient */}
          <div style={{
            position:'absolute', top:'10%', left:'50%', transform:'translateX(-50%)',
            width:520, height:520, borderRadius:'50%',
            background:`radial-gradient(circle, ${sc}14 0%, ${sc}06 40%, transparent 70%)`,
            transition:'background 1.2s ease',
            /* No filter:blur — use pure CSS gradient instead */
          }}/>
        </div>
      )}
      ...
      <div
        style={callFullscreen ? {
          ...
          width:300, touchAction:'none', willChange:'transform',
          /* no drop-shadow — use boxShadow on inner div only */
        }}
        ...
        {callFullscreen ? (
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{display:'flex', alignItems:'center', gap:8}}>
                <div style={{width:7, height:7, borderRadius:'50%', background:sc,
                  ...
                  animation: callStatus==='listening'?'callRing 1.6s ease-out infinite':'none',
                  willChange:'transform',
                }}/>
                <span style={{fontSize:11, fontWeight:700, color:sc, letterSpacing:'0.12em', textTransform:'uppercase'}}>{sl}</span>
              </div>
              <span style={{fontSize:12, fontWeight:600, color:'rgba(255,255,255,0.3)', fontVariantNumeric:'tabular-nums'}}>
                {fmt(callDuration)}
              </span>
              <button onClick={()=>setCallFullscreen(false)}
                style={{background:'rgba(255,255,255,0.08)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:8,padding:'4px 10px',color:'rgba(255,255,255,0.5)',fontSize:10,cursor:'pointer',letterSpacing:'0.04em'}}>
                ⊡ Window
              </button>
            </div>
            ...
            <div style={{display:'flex', flexDirection:'column', alignItems:'center', flexShrink:0}}>
              {/* Pulse rings — CSS keyframe only, no JS, will-change:transform for compositor */}
              <div style={{position:'relative', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:20}}>
                {[116,154,192].map((sz,ri)=>(
                  <div key={ri} style={{
                    position:'absolute', width:sz, height:sz, borderRadius:'50%',
                    border:`1px solid ${sc}`,
                    opacity: isActive ? [0.45,0.22,0.10][ri] : 0.05,
                    animation: isActive ? `callRing ${1.8+ri*0.5}s ease-out infinite` : 'none',
                    animationDelay:`${ri*0.55}s`,
                ...
                <div style={{
                  ...
                  border:`3px solid ${sc}`,
                  boxShadow:`0 0 0 4px ${sc}18, 0 0 40px ${sc}50`,
                  transition:'border-color 0.4s, box-shadow 0.4s',
                  position:'relative',
                }}>
                  <img src={avatarSrc} alt="Tessa" style={{width:'100%',height:'100%',objectFit:'cover'}}
                    onError={e=>{(e.currentTarget as HTMLImageElement).src='/avatars/cosmic.png';}}/>
                  {/* Speaking indicator — border glow only, no inner overlay animation */}
                </div>
              </div>

              <p style={{color:'#fff',fontWeight:900,fontSize:22,letterSpacing:'0.08em',margin:'0 0 4px',fontFamily:'inherit'}}>TESSA</p>
              <p style={{color:'rgba(255,255,255,0.3)',fontSize:11,margin:0,letterSpacing:'0.04em'}}>
                {isCreatorMode?'💝 Personal AI':'AI Voice Call'}
              </p>
            </div>

            {/* Waveform — fixed heights, scaleY only = zero layout recalc, GPU compositor */}
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:4,height:60,width:'100%',padding:'0 28px',flexShrink:0}}>
              {barPattern.slice(0,28).map((bh,i)=>(
                <div key={i} className="wave-bar" style={{
                  width:5, height:Math.max(6,bh), borderRadius:3,
                  background: isActive ? sc : 'rgba(255,255,255,0.10)',
                  transformOrigin:'center',
                  transform: isActive ? 'scaleY(1)' : 'scaleY(0.15)',
                  animation: isActive ? `soundBar ${0.65+Math.sin(i*1.1)*0.30}s ease-in-out infinite alternate` : 'none',
            ...
            {/* Transcript — GPU-safe, no backdrop blur, contain:strict */}
            <div style={{
              width:'100%', maxWidth:480, padding:'0 20px', flexShrink:0,
              contain:'strict', height:172,
            }}>
              <div style={{
                height:'100%', overflowY:'auto', borderRadius:18,
                background:'rgba(255,255,255,0.06)',
                border:'1px solid rgba(255,255,255,0.09)',
                padding:'12px 16px',
                /* NO backdropFilter — causes scroll jank */
                scrollBehavior:'auto',
                overscrollBehavior:'contain',
              }}>
                {callTranscript.length===0 ? (
                  <p style={{fontSize:12,color:'rgba(255,255,255,0.22)',textAlign:'center',margin:0,letterSpacing:'0.02em'}}>
                    Your conversation appears here…
                  </p>
                ) : (
                  [...callTranscript].slice(-8).map((m,i,arr)=>(
                    <div key={m.role+i} style={{marginBottom: i<arr.length-1?10:0}}>
                      <span style={{
                        display:'inline-block', fontSize:9, fontWeight:800,
                        letterSpacing:'0.1em', textTransform:'uppercase',
                        color: m.role==='user'?'rgba(255,255,255,0.38)':sc,
                        marginBottom:2,
                      }}>
                        {m.role==='user'?'You':'Tessa'}
                      </span>
                      <p style={{
                        fontSize:13, fontWeight:m.role==='user'?400:500,
                        color:m.role==='user'?'rgba(255,255,255,0.78)':'#fff',
                ...
                {interimText && (
                  <p style={{fontSize:12,color:sc,fontStyle:'italic',margin:'6px 0 0',opacity:0.75}}>
                    {interimText}…
                  </p>
                )}
              </div>
            ...
            {/* Bottom controls */}
            <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:10, paddingBottom:32, flexShrink:0}}>
              {/* Hint text */}
              <p style={{
                color: isActive ? sc : 'rgba(255,255,255,0.25)',
                fontSize:11, fontWeight:600, letterSpacing:'0.06em',
                textTransform:'uppercase', transition:'color 0.4s', margin:0,
              }}>
                {callStatus==='listening'?'🎤 Listening — speak now':callStatus==='thinking'?'🧠 Thinking…':callStatus==='speaking'?'🔊 Speaking…':''}
              </p>
              {/* End call */}
              <button onClick={endVoiceCall}
                style={{
                  width:68, height:68, borderRadius:'50%', border:'none', cursor:'pointer',
                  background:'linear-gradient(135deg,#ff3b30,#c9000f)',
                  boxShadow:'0 8px 28px rgba(255,59,48,0.55), 0 0 0 4px rgba(255,59,48,0.15)',
              ...
              <p style={{color:'rgba(255,255,255,0.18)',fontSize:9,margin:0,letterSpacing:'0.06em'}}>TAP TO END CALL</p>
            </div>
          </>
        ) : (
          /* ══ MINI WINDOW LAYOUT ════════════════════════════════════════════ */
          <div style={{
            borderRadius:28, overflow:'hidden', width:300,
            background:'linear-gradient(160deg,#0f0c24 0%,#1a1035 50%,#0c1a3a 100%)',
            border:`1px solid ${sc}28`,
            boxShadow:`0 0 0 1px ${sc}12, inset 0 1px 0 rgba(255,255,255,0.06)`,
            transition:'border-color 0.5s',
          }}>
            {/* Ambient glow */}
            <div style={{position:'absolute',top:-30,left:'50%',transform:'translateX(-50%)',width:160,height:160,borderRadius:'50%',background:sc,filter:'blur(50px)',opacity:0.1,transition:'background 0.5s',pointerEvents:'none'}}/>

            {/* Top controls */}
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 14px 6px',cursor:'grab'}}>
              <div style={{display:'flex',alignItems:'center',gap:6}}>
                <div style={{width:6,height:6,borderRadius:'50%',background:sc,boxShadow:`0 0 6px ${sc}`,animation:'pulse 1.4s ease-in-out infinite'}}/>
                <span style={{fontSize:10,fontWeight:600,color:sc,letterSpacing:'0.08em',textTransform:'uppercase'}}>{sl}</span>
              </div>
              <span style={{fontSize:10,color:'rgba(255,255,255,0.3)',fontVariantNumeric:'tabular-nums'}}>{fmt(callDuration)}</span>
              <button onClick={()=>setCallFullscreen(true)}
                style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:6,padding:'3px 7px',color:'rgba(255,255,255,0.45)',fontSize:9,cursor:'pointer'}}>
                ⛶ Full
              </button>
            </div>

            <div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'4px 16px 18px'}}>
              {/* Mini avatar */}
              <div style={{position:'relative',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:10}}>
                {[72,96].map((sz,ri)=>(
                  <div key={ri} style={{position:'absolute',width:sz,height:sz,borderRadius:'50%',border:`1px solid ${sc}`,opacity:isActive?[0.35,0.15][ri]:0.05,animation:isActive?`callRing ${1.8+ri*0.5}s ease-out infinite`:'none',animationDelay:`${ri*0.55}s`,willChange:'transform,opacity'}}/>
                ))}
                <div style={{width:62,height:62,borderRadius:'50%',overflow:'hidden',border:`2.5px solid ${sc}`,boxShadow:`0 0 20px ${sc}40`,position:'relative',transition:'border-color 0.4s'}}>
                  <img src={avatarSrc} alt="Tessa" style={{width:'100%',height:'100%',objectFit:'cover'}}
                    onError={e=>{(e.currentTarget as HTMLImageElement).src='/avatars/cosmic.png';}}/>
                </div>
              </div>

              <p style={{color:'#fff',fontWeight:900,fontSize:14,letterSpacing:'0.08em',margin:'0 0 1px'}}>TESSA</p>
              <p style={{color:'rgba(255,255,255,0.3)',fontSize:9,margin:'0 0 10px'}}>{isCreatorMode?'💝 Creator Mode':'AI Voice Call'}</p>

              {/* Mini waveform — scaleY only, no height = no layout recalc */}
              <div style={{display:'flex',alignItems:'center',gap:2.5,height:28,marginBottom:10}}>
                {barPattern.slice(0,22).map((bh,i)=>(
                  <div key={i} className="wave-bar" style={{
                    width:3, height:Math.max(5,Math.round(bh*0.72)), borderRadius:2,
                    background: isActive ? sc : 'rgba(255,255,255,0.14)',
                    transformOrigin:'center',
                    transform: isActive ? 'scaleY(1)' : 'scaleY(0.15)',
                    animation: isActive ? `soundBar ${0.65+Math.sin(i*1.1)*0.3}s ease-in-out infinite alternate` : 'none',
                    animationDelay:`${(i*0.055)%0.75}s`,
              ...
              {/* Mini transcript */}
              <div style={{width:'100%',maxHeight:56,overflowY:'auto',borderRadius:12,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.06)',padding:'6px 8px',marginBottom:12}}>
                {callTranscript.length===0 ? (
                  <p style={{fontSize:9,color:'rgba(255,255,255,0.2)',textAlign:'center',margin:0}}>Conversation appears here…</p>
                ) : (
                  [...callTranscript].slice(-3).map((m,i)=>(
                    <p key={i} style={{fontSize:9,color:m.role==='user'?'rgba(255,255,255,0.65)':sc,margin:'0 0 2px',lineHeight:1.4}}>
                      <strong>{m.role==='user'?'You: ':'T: '}</strong>{m.text.slice(0,80)}{m.text.length>80?'…':''}
                    </p>
                  ))
                )}
                {interimText && <p style={{fontSize:9,color:'rgba(255,255,255,0.3)',fontStyle:'italic',margin:'2px 0 0'}}>{interimText}…</p>}
              </div>

              {/* End call */}
              <button onClick={endVoiceCall}
                style={{width:52,height:52,borderRadius:'50%',border:'none',cursor:'pointer',background:'linear-gradient(135deg,#ff3b30,#c9000f)',boxShadow:'0 4px 16px rgba(255,59,48,0.5)',display:'flex',alignItems:'center',justifyContent:'center',transition:'transform 0.15s'}}
              ...
              <p style={{color:'rgba(255,255,255,0.15)',fontSize:8,marginTop:5,letterSpacing:'0.04em'}}>end call</p>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div className={`h-screen ${t.text} flex overflow-hidden relative ${fontSizeClass}`}
      ...
      @keyframes soundBar {
        ...
        100% { transform: scaleY(0.28); opacity: 0.45; }
      }
      /* Voice call ring — slow, smooth, organic */
      @keyframes callRing {
        0%   { transform: scale(0.9);  opacity: 0.7; }
        60%  { transform: scale(1.12); opacity: 0.2; }
      ...
      @media (max-width: 767px) {
        ...
        *, *::before, *::after { transition-duration: 0.05s !important;
        ...
        /* EXEMPTIONS — wave bars and call rings must animate */
        .wave-bar, .call-ring {
...
`}</style>
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0 relative z-10" style={{height:"100dvh"}}>
        ...
        {/* ── MESSAGES AREA ──────────────────────────────────────────────── */}
        <div className="flex-1 overflow-y-auto px-2 md:px-5 py-4 scroll-smooth">
              ...
              <div className="space-y-2.5 pb-2">
                ...
                {messages.map((msg,idx)=>{
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`msg-row group flex gap-2 ${isUser?'justify-end':'justify-start'} ${isGrouped?'mt-0.5':'mt-3'} ${isUser?'pl-8':'pr-1'}`}
                    >
                      {/* LEFT: Tessa avatar — chosen png, no border, soft bottom fade */}
                      {!isUser && (
                        <div className="flex-shrink-0 flex flex-col justify-end" style={{width:32}}>
                          {!isGrouped ? (
                            <div style={{
                              width:32, height:36, flexShrink:0, marginBottom:2,
                              ...
                              <img
                                ...
                                onError={e=>{(e.currentTarget as HTMLImageElement).src='/avatars/cosmic.png';}}
                              />
                              {/* Subtle accent glow at base */}
                              {animations && (
                                <div style={{
                                  position:'absolute', bottom:0, left:0, right:0, height:8,
                                  background:`linear-gradient(to top, ${t.glow}30, transparent)`,
                                  pointerEvents:'none',
                                }}/>
                              )}
                            </div>
                      ...
                      {/* BUBBLE */}
                      <div className={`flex flex-col ${isUser?'items-end max-w-[86%]':'items-start w-full'}`}>
