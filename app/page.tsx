...
Style: Direct, warm, specific. No generic advice. Use actual numbers from his data. Max 200 words total. No fluff.`;
  ...
  const startVoiceCall = () => {
    ...
    // Median app (iOS/Android WebView): trigger mic through standard getUserMedia
    // iOS requires NSMicrophoneUsageDescription in Info.plist (set via Median dashboard)
    // Android requires RECORD_AUDIO permission (auto-granted via Median config)
    const micPromise = navigator.mediaDevices?.getUserMedia({ audio: true })
      ?? new Promise((_,rej) => rej(new Error('MediaDevices not available')));
    micPromise.then(() => {
      playRingTone();
      ...
      const isApp = /Median|median|gonative|GoNative/i.test(navigator.userAgent||'');
      if (isApp) {
        alert('Microphone permission needed. Go to your phone Settings → Apps → TESSA → Permissions → Microphone → Allow, then try again.');
      } else {
  ...
  const VoiceCallWindow = () => {
    ...
    return (
      ...
      <div
        ...
        onPointerDown={callFullscreen ? undefined : onPtrDown}
        onPointerMove={callFullscreen ? undefined : onPtrMove}
        onPointerUp={callFullscreen ? undefined : onPtrUp}
        onPointerCancel={callFullscreen ? undefined : onPtrUp}
      >
        <div style={{
          borderRadius: callFullscreen ? 0 : 28,
          overflow: 'hidden',
          width: callFullscreen ? '100%' : undefined,
          maxWidth: callFullscreen ? 520 : undefined,
          background: 'linear-gradient(160deg, #0f0c24 0%, #1a1035 40%, #0c1a3a 100%)',
          border: `1px solid ${statusColor}30`,
          boxShadow: callFullscreen ? 'none' : `0 0 0 1px ${statusColor}18, inset 0 1px 0 rgba(255,255,255,0.06)`,
          ...
          {/* Drag pill + fullscreen toggle */}
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 14px 4px', cursor:'grab' }}>
            <button onClick={()=>setCallFullscreen(p=>!p)}
              style={{ background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.12)',
                borderRadius:6, padding:'3px 8px', color:'rgba(255,255,255,0.5)', fontSize:10, cursor:'pointer' }}>
              {callFullscreen ? '⊡ Window' : '⛶ Full'}
            </button>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'6px 20px 20px' }}>
            ...
            <div style={{
              width:'100%', maxHeight: callFullscreen ? 220 : 72, overflowY:'auto', borderRadius:14,
            ...
            {callFullscreen && callStatus==='listening' && (
              <p style={{ color: statusColor, fontSize:11, marginTop:10, fontWeight:600, letterSpacing:'0.06em', textTransform:'uppercase', opacity:0.8 }}>
                🎤 Listening — speak now
              </p>
            )}
            {callFullscreen && callStatus==='idle' && (
          ...
          <div className="flex items-center justify-between gap-2 h-[60px]">
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[9px] mt-0.5 font-mono tabular-nums" style={{color:t.glow,opacity:0.7,letterSpacing:'0.04em'}}>
                  ...
                  {ambientState!=='idle' && (
                    <span className="ml-1.5 inline-flex items-center gap-0.5 px-1 py-0.5 rounded-md text-[8px] font-bold"
                      style={{
          ...
          <div className={`flex-shrink-0 ${t.bar} px-3 md:px-6 py-3 safe-bottom`}>
            <div className="max-w-2xl mx-auto w-full">
              ...
              <div className="mt-2 flex items-center gap-1 overflow-x-auto pb-0.5"
                ...
                <button onClick={()=>setWebSearchOn(p=>!p)}
                  ...
                  style={{
                    background: webSearchOn
                      ? `linear-gradient(135deg,${t.glow}30,${t.glow}18)`
                      : t.isLight ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.05)',
                    border: `1px solid ${webSearchOn ? t.glow+'60' : t.isLight?'rgba(0,0,0,0.1)':'rgba(255,255,255,0.1)'}`,
                    boxShadow: webSearchOn ? `inset 0 1px 0 rgba(255,255,255,0.15), 0 1px 4px ${t.glow}20` : 'inset 0 1px 0 rgba(255,255,255,0.06)',
                  }}>
                ...
                {/* Divider */}
                <div className="flex-shrink-0 w-px h-4 mx-0.5" style={{background: t.isLight?'rgba(0,0,0,0.12)':'rgba(255,255,255,0.1)'}}/>
                ...
                {[
                  ...
                  {ico:'📒', lbl:'Notes', fn:()=>setShowShortNotes(true), badge:shortNotes.length>0?shortNotes.length:0},
                  {ico:'📬', lbl:'Capsule',  fn:()=>setShowTimeCapsule(true)},
                  {ico:'📰', lbl:'News',     fn:()=>setShowNewsTicker(p=>!p), active:showNewsTicker, title:'Live BBC news ticker — click any headline to ask Tessa about it'},
                ].map(btn=>(
                  <button key={btn.lbl} onClick={btn.fn}
                    ...
                    style={{
                      background: btn.active
                        ? `linear-gradient(135deg,${t.glow}28,${t.glow}14)`
                        : t.isLight ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.05)',
                      border: `1px solid ${btn.active ? t.glow+'50' : t.isLight?'rgba(0,0,0,0.09)':'rgba(255,255,255,0.09)'}`,
                      boxShadow: btn.active ? `inset 0 1px 0 rgba(255,255,255,0.15), 0 1px 4px ${t.glow}18` : 'inset 0 1px 0 rgba(255,255,255,0.06)',
                    }}>
                    <span style={{fontSize:11, lineHeight:1}}>{btn.ico}</span>
                    <span className="hidden sm:block text-[10px] font-medium"
                      style={{color: btn.active?t.glow:t.isLight?'#64748b':'#94a3b8'}}>
                      {btn.lbl}
                    </span>
                    {btn.badge > 0 && (
                ...
                {lastSearchData && (
                  <>
                    <div className="flex-shrink-0 w-px h-4 mx-0.5" style={{background: t.isLight?'rgba(0,0,0,0.12)':'rgba(255,255,255,0.1)'}}/>
                    <button onClick={()=>setShowSearchPanel(p=>!p)}
                      className="flex-shrink-0 flex items-center gap-1.5 h-7 px-3 rounded-full transition-all active:scale-95"
                      style={{
                        background: showSearchPanel ? `${t.glow}20` : t.isLight?'rgba(0,0,0,0.05)':'rgba(255,255,255,0.05)',
                        border: `1px solid ${t.glow}30`,
                        boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.06)',
                      }}>
                      <ExternalLink size={10} style={{color:t.glow}}/>
                      <span className="text-[10px] font-medium" style={{color:t.glow}}>
                        Sources ({lastSearchData.sources.length})
                      </span>
                    </button>
                  </>
                )}

                {/* Right hint — pushes to end */}
                ...
                {isCreatorMode && !isRecording && (
                  <span className="flex-shrink-0 text-[9px] font-medium text-pink-400/50 flex items-center gap-1">
