  ...
  return (
    <div className={`h-screen ${t.bg} ${t.text} flex overflow-hidden relative transition-colors duration-500 ${fontSizeClass}`}>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden min-w-0 relative z-10">
        ...
        {/* ── WELLNESS RIBBON — elegant inline expand below header ── */}
        {showWellness && isCreatorMode && (
          <div className={`flex-shrink-0 mx-3 md:mx-5 mb-0 overflow-hidden`}
            style={{animation:'wellness-expand 0.35s cubic-bezier(0.16,1,0.3,1)'}}>
            <div className={`rounded-2xl overflow-hidden`}
              style={{background:`${t.glow}07`,border:`1px solid ${t.glow}18`,marginBottom:'8px'}}>
              {/* Ribbon header */}
              ...
              {/* Content */}
              <div className={`px-4 pb-3 border-t ${t.div} pt-2.5`}>
          ...
          <div className="max-w-2xl mx-auto w-full">
            ...
            {showDashboard&&isCreatorMode ? (
              ...
              <div className="space-y-2.5 pb-2">
                ...
                {/* ── TESSA INSIGHTS RIBBON — shown inline when chat is active ── */}
                {isCreatorMode && messages.length > 0 && (
                  <div className={`rounded-2xl overflow-hidden mb-1 transition-all`}
                    style={{background:`${t.glow}08`,border:`1px solid ${t.glow}16`}}>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className="w-5 h-5 rounded-lg overflow-hidden flex-shrink-0 border"
                        ...
                        <span className={`text-[10px] font-black tracking-wide`} style={{color:t.glow}}>
                          Tessa's Insights
                        </span>
                        <span className={`text-[9px] ${t.sub}`}>· {moodEmoji} {moodLabel}</span>
                      </div>
                      <div className={`flex items-center gap-1.5 ${t.sub}`}>
                        <span className="text-[9px]">{insightsOpen?'collapse':'expand'}</span>
                        {insightsOpen ? <ChevronUp size={10}/> : <ChevronDown size={10}/>}
                      </div>
                    </button>
                    {insightsOpen && (
                      <div className={`px-3.5 pb-3 border-t ${t.div}`}>
