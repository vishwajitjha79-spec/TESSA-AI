...
import {
  ...
  Bell, BellOff, Eye, EyeOff, Palette, RotateCcw,
  Languages, Monitor, Smartphone, Download, Upload,
  Lock, Unlock, Cpu, BarChart3, Hash, ChevronRight,
} from 'lucide-react';
...
const THEMES = {
  ...
  // ── PASTEL: soft daylight lavender — truly light theme ──────────────────
  pastel: {
    ...
    settBg:'bg-white/97 backdrop-blur-2xl border-r border-violet-200',
    settBgC:'bg-white/97 backdrop-blur-2xl border-r border-fuchsia-200',
    settText:'text-violet-700', settTextHover:'hover:text-violet-900',
    settSub:'text-violet-400', settLabel:'text-violet-400',
    settCard:'bg-violet-50 border border-violet-200',
    settActive:'bg-violet-100 border border-violet-300',
    isLight:true,
  },

  // ── SAKURA: soft cherry-blossom daylight — truly light theme ────────────
  sakura: {
    ...
    settBg:'bg-white/97 backdrop-blur-2xl border-r border-rose-200',
    settBgC:'bg-white/97 backdrop-blur-2xl border-r border-pink-200',
    settText:'text-rose-700', settTextHover:'hover:text-rose-900',
    settSub:'text-rose-400', settLabel:'text-rose-400',
    settCard:'bg-rose-50 border border-rose-200',
    settActive:'bg-rose-100 border border-rose-300',
    isLight:true,
  },

  // ── ANKIT'S SPECIAL: dawn gold — warm cream + rich gold, light theme ─────
  ankit: {
    ...
    settBg:'bg-white/97 backdrop-blur-2xl border-r border-amber-200',
    settBgC:'bg-white/97 backdrop-blur-2xl border-r border-orange-200',
    settText:'text-amber-700', settTextHover:'hover:text-amber-900',
    settSub:'text-amber-400', settLabel:'text-amber-400',
...
// Aurora background for dark themes
function AuroraBg({ glow, glow2, theme }: { glow: string; glow2: string; theme: Theme }) {
  const isAnkit = theme === 'ankit';
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0" aria-hidden>
...
// Light theme gradient mesh background
function LightBg({ creator, theme }: { creator: boolean; theme: Theme }) {
  const meshColors: Record<Theme, [string, string, string]> = {
    ...
    // fallbacks for non-light themes (shouldn't be used)
    dark: ['#dbeafe','#ede9fe','#f0f9ff'],
    cyberpunk: ['#dbeafe','#ede9fe','#f0f9ff'],
    ocean: ['#dbeafe','#ede9fe','#f0f9ff'],
    sunset: ['#dbeafe','#ede9fe','#f0f9ff'],
  };
...
// ─────────────────────────────────────────────────────────────────────────────
// INLINE INSIGHTS PANEL — floats above input bar in the chat area
// ─────────────────────────────────────────────────────────────────────────────
function InsightsFloatingCard({ t, glow, glow2, isCreatorMode, onClose }: {
  t: ReturnType<typeof useT>; glow: string; glow2: string;
  isCreatorMode: boolean; onClose: () => void;
}) {
  return (
    <div
      className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-2xl px-3 md:px-6 pb-2 z-20 animate-fadeIn"
      style={{ pointerEvents: 'none' }}
    >
      <div
        className={`rounded-2xl overflow-hidden border shadow-xl ${t.panel}`}
        style={{
          boxShadow: `0 -4px 32px ${glow}12, 0 8px 24px rgba(0,0,0,0.15)`,
          pointerEvents: 'auto',
        }}
      >
        {/* Header */}
        <div className={`flex items-center justify-between px-4 py-2.5 border-b ${t.div}`}>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[11px] font-bold ${t.accent}`}>Tessa Insights</span>
            <span className={`text-[9px] px-2 py-0.5 rounded-full font-medium`}
              style={{ background: `${glow}15`, color: glow, border: `1px solid ${glow}25` }}>
              Live
            </span>
          </div>
          <button onClick={onClose}
            className={`p-1 rounded-lg transition-colors ${t.isLight ? 'hover:bg-black/5' : 'hover:bg-white/8'} ${t.sub}`}>
            <X size={12} />
          </button>
        </div>
        {/* Content */}
...
export default function Home() {
  ...
  const [showReportCard,   setShowReportCard]   = useState(false);
  const [notesExpanded,    setNotesExpanded]    = useState(true);
  // NEW: inline insights panel above input bar
  const [showInsights,     setShowInsights]     = useState(false);
  // NEW: wellness panel in right sidebar (collapsible)
  ...
  return (
    <div className={`h-screen ${t.bg} ${t.text} flex overflow-hidden relative transition-colors duration-500 ${fontSizeClass}`}>
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          LEFT SIDEBAR — New Chat at top, account/stats at bottom
      ═══════════════════════════════════════════════════════════════════ */}
      <aside
        className={`fixed md:relative inset-y-0 left-0 z-30 md:z-10 flex-shrink-0 flex flex-col h-screen border-r transition-all duration-300 ease-in-out ${t.panel} ${showSidebar ? 'w-[270px] translate-x-0' : 'w-[270px] -translate-x-full md:w-0 md:translate-x-0 overflow-hidden'}`}
        style={{backdropFilter:'blur(24px)'}}
      ...
      >
        <div className="flex flex-col h-full w-[270px]">
          ...
          {/* ── NEW CHAT — prominent, at the very top ── */}
          <div className={`flex-shrink-0 px-3 pt-3 pb-2`}>
            <button onClick={startNewChat}
              className={`w-full py-2.5 rounded-xl text-[12px] font-bold flex items-center justify-center gap-2 transition-all active:scale-98 shadow-sm ${t.btnP}`}>
          ...
          <div className="flex-shrink-0">
            <button onClick={()=>setNotesExpanded(p=>!p)}
              className={`w-full flex items-center justify-between px-4 py-2 text-[11px] font-semibold ${t.accent} hover:bg-white/[0.03] transition-colors`}>
              <span className="flex items-center gap-2"><StickyNote size={12} />Quick Notes</span>
              {notesExpanded ? <ChevronUp size={11} /> : <ChevronDown size={11} />}
            </button>
            {notesExpanded && (
          ...
          {/* ── CONVERSATIONS ── */}
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
            <div className={`flex-shrink-0 px-4 pt-2 pb-1.5 flex items-center justify-between border-b ${t.div}`}>
              <p className={`text-[9px] font-black tracking-[0.18em] uppercase ${t.sub}`}>
                {isCreatorMode ? '💬 Our Chats' : '💬 History'}
              </p>
              <span className={`text-[9px] ${t.sub}`}>{shownConvs.length}</span>
            </div>
          ...
          {/* ── BOTTOM: Stats + Account ── */}
          <div className={`flex-shrink-0 border-t ${t.div}`}>
            {/* Memory stat */}
            <div className={`px-4 py-2 flex items-center gap-3 border-b ${t.div}`}>
              <div className="flex items-center gap-1">
                <Brain size={9} className={t.sub} /><span className={`text-[9px] ${t.sub}`}>{getAllMemories().length} memories</span>
              </div>
              {user&&!isGuest&&(
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-4 space-y-5">
              ...
              {settingsTab==='appearance' && (<>
                <div>
                  ...
                  <p className={`text-[9px] mt-1.5 ${t.sSub}`}>✦ Light themes</p>
                </div>
              ...
              {settingsTab==='about' && (<>
                ...
                <div className={`space-y-2`}>
                  {[
                    ...
                    ['Theme', theme==='ankit'?"✨ Ankit's":theme.charAt(0).toUpperCase()+theme.slice(1)],
                    ['Font Size', fontSize],
                    ['Language', language],
                  ].map(([k,v])=>(
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden min-w-0 relative z-10">
        ...
        <header className={`flex-shrink-0 ${t.header} px-3 md:px-5`}>
          <div className="flex items-center justify-between gap-2 h-[60px]">
            ...
            <div className="flex items-center gap-0.5 flex-shrink-0">
              ...
              {/* ── INSIGHTS button (creator mode) — in header ── */}
              {isCreatorMode && (
                <button
                  onClick={() => setShowInsights(p => !p)}
                  title="Tessa Insights"
                  className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${showInsights ? 'text-white' : 'hover:bg-white/[0.07] ' + t.sub}`}
                  style={showInsights ? { background: `${t.glow}14`, outline: `1px solid ${t.glow}25` } : {}}
                >
                  <Sparkles size={16} />
                </button>
              )}

              {/* ── WELLNESS button (creator mode) — in header, collapsible panel ── */}
              {isCreatorMode && (
                <button
                  onClick={() => setShowWellnessPanel(p => !p)}
                  title="Wellness"
                  className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${showWellnessPanel ? 'text-white' : 'hover:bg-white/[0.07] ' + t.sub}`}
                  style={showWellnessPanel ? { background: `${t.glow}14`, outline: `1px solid ${t.glow}25` } : {}}
                >
                  <Activity size={16} />
                </button>
              )}
              ...
              {/* Dashboard */}
              {isCreatorMode&&(
                <button onClick={()=>setShowDashboard(p=>!p)} title="Full Dashboard"
        ...
        {/* ── WELLNESS COLLAPSIBLE PANEL (drops from header) ── */}
        {showWellnessPanel && isCreatorMode && (
          <div className={`flex-shrink-0 border-b ${t.div} px-4 py-3 animate-fadeIn`}
            style={{ background: t.isLight ? 'rgba(255,255,255,0.7)' : 'rgba(0,0,0,0.2)', backdropFilter: 'blur(16px)' }}>
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <Activity size={11} style={{ color: t.glow }} />
                  <span className={`text-[10px] font-bold ${t.accent}`}>Daily Wellness</span>
                </div>
                <button
                  onClick={() => { addWater(1); setWellnessVersion(v => v + 1); }}
                  className={`text-[9px] px-2.5 py-1 rounded-full flex items-center gap-1 transition-all active:scale-90 ${t.btnS}`}
                  style={{ border: `1px solid ${t.glow}20` }}
                >
                  <Droplets size={9} className="text-blue-400" /> +💧 Water
                </button>
              </div>
              <DailyWellness isCreatorMode={isCreatorMode} refreshTrigger={wellnessVersion} />
            </div>
        ...
        {!showDashboard&&(
          <div className={`flex-shrink-0 relative`}>
            ...
            {/* ── INSIGHTS FLOATING CARD — sits above the input bar ── */}
            {showInsights && isCreatorMode && (
              <div className={`absolute bottom-full left-0 right-0 px-3 md:px-6 pb-2 z-20 animate-fadeIn`}>
                <div className="max-w-2xl mx-auto">
                  <div className={`rounded-2xl overflow-hidden border shadow-2xl ${t.panel}`}
                    style={{boxShadow:`0 -8px 40px ${t.glow}12, 0 4px 24px rgba(0,0,0,0.2)`}}>
                    <div className={`flex items-center justify-between px-4 py-2.5 border-b ${t.div}`}>
                      <div className="flex items-center gap-2">
                        <Sparkles size={12} style={{color:t.glow}}/>
                        <span className={`text-[11px] font-bold ${t.accent}`}>Tessa Insights</span>
                        <span className={`text-[9px] px-2 py-0.5 rounded-full font-medium`}
                          style={{background:`${t.glow}15`,color:t.glow,border:`1px solid ${t.glow}25`}}>
                          Live
                        </span>
                      </div>
            ...
            <div className={`${t.bar} px-3 md:px-6 py-3`}>
              <div className="max-w-2xl mx-auto w-full">
