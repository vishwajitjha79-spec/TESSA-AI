...
const CHANGELOG = [
  ...
  },
  {
    ...
    updates: [
      '🎨 Ankit theme (rose-cream + blue/black palette)',
...
Style: Direct, warm, specific. No generic advice. Use actual numbers from his data. Max 200 words total. No fluff.`;
  ...
  const callSpeak = (text: string, onDone: () => void) => {
    ...
    const clean = text.replace(/[*_~`#]/g,'').replace(/TESSA/gi,'').replace(/\n+/g,' ').trim().slice(0,280);
    const lang = language==='hi'?'hi-IN':'en-US';
    const utter = new SpeechSynthesisUtterance(clean);
    utter.lang = lang; utter.pitch = 1.7; utter.rate = 1.2; utter.volume = 0.95;
    const doSpeak = () => {
      const vs = window.speechSynthesis.getVoices();
      const v = vs.find(v=>/zira|google us english female/i.test(v.name))
             ?? vs.find(v=>/google.*english/i.test(v.name)&&/female/i.test(v.name))
             ?? vs.find(v=>/nicky|aria|jenny|samantha/i.test(v.name))
             ?? vs.find(v=>v.lang.startsWith('en')&&/female/i.test(v.name))
             ?? null;
      if (v) utter.voice = v;
      // Android Chrome keep-alive: resume if paused
      const ka = setInterval(() => {
        try { if (window.speechSynthesis.speaking && window.speechSynthesis.paused) window.speechSynthesis.resume(); } catch {}
      }, 250);
      utter.onend   = () => { clearInterval(ka); isSpeakingRef.current = false; onDone(); };
      utter.onerror = () => { clearInterval(ka); isSpeakingRef.current = false; onDone(); };
      window.speechSynthesis.speak(utter);
    };
    if (window.speechSynthesis.getVoices().length) { doSpeak(); }
    else { window.speechSynthesis.onvoiceschanged = () => { voicesReady.current = true; doSpeak(); }; }
  };
  ...
  const callListen = () => {
    ...
    // Stop any previous recognition safely
    if (recognitionRef.current) {
    ...
    r.continuous      = false;   // mobile-safe: let browser detect silence
    ...
    r.onstart = () => { setCallStatus('listening'); setIsRecording(true); };
    ...
    r.onspeechend = () => {
      // Give 400ms grace for trailing words before stopping
      silenceTimer = setTimeout(() => { try { r.stop(); } catch {} }, 400);
    };
    ...
    r.onend = async () => {
      ...
      if (!said || said.length < 2) { setTimeout(callListen, 300); return; }
      ...
      try {
        const res = await fetch('/api/chat', {
          ...
          body: JSON.stringify({
            ...
            maxTokens: 60,
            _systemOverride: `Voice call. ONE short sentence max. No markdown. Warm and natural.${isCreatorMode?' Ankit\'s personal AI.':''}`,
          }),
        });
        const data = await res.json();
        ...
        callSpeak(reply, () => { if (callLoopRef.current) setTimeout(callListen, 150); });
      } catch {
        callSpeak("Lost signal for a moment!", () => { if (callLoopRef.current) setTimeout(callListen, 800); });
      }
    };

    r.onerror = (e: any) => {
      setIsRecording(false);
      if (e.error==='not-allowed'||e.error==='audio-capture') { endVoiceCall(); return; }
      // no-speech / network = retry after short pause
      if (callLoopRef.current) setTimeout(callListen, 400);
    };

    recognitionRef.current = r;
    try { r.start(); } catch { if (callLoopRef.current) setTimeout(callListen, 500); }
  };
