...
Style: Direct, warm, specific. No generic advice. Use actual numbers from his data. Max 200 words total. No fluff.`;
  ...
  const callSpeakTessa = (text: string, onDone: () => void) => {
    ...
    // Resume suspended speech (iOS Safari + fullscreen can suspend it)
    ...
    const clean = text.replace(/\*\*/g,'').replace(/#{1,6}\s/g,'').replace(/[*_~`]/g,'')
                      .replace(/\bTESSA\b/gi,'').slice(0, 400);
    const lang = language === 'hi' ? 'hi-IN' : 'en-IN';
    const u = new SpeechSynthesisUtterance(clean);
    u.pitch = 1.85; u.rate = 1.18; u.volume = 0.92; u.lang = lang;
    const trySpeak = () => {
      const v = getBestVoice(lang); if (v) u.voice = v;
      // Workaround: Chrome on Android sometimes pauses long utterances — keep-alive ping
      ...
      u.onend  = () => { clearInterval(keepAlive); isSpeakingRef.current = false; onDone(); };
      u.onerror = (e:any) => {
        clearInterval(keepAlive);
        isSpeakingRef.current = false;
        // 'interrupted' is normal on cancel — still call onDone
  ...
  const startCallListening = () => {
    ...
    r.onend = async () => {
      ...
      if (!said || said.length < 2) { setTimeout(startCallListening, 350); return; }
      ...
      try {
        const res = await fetch('/api/chat', {
          ...
          body: JSON.stringify({
            ...
            maxTokens: 150,
            _systemOverride: `You are Tessa in a live voice call. Reply in 1-2 short sentences only. No markdown, no bullet points, no asterisks. Speak like a real person — warm, quick, natural.${isCreatorMode ? " You're Ankit's personal AI — be caring and direct." : ''}`,
          }),
        });
        const data = await res.json();
        ...
        callSpeakTessa(reply, () => {
          if (callLoopRef.current) setTimeout(startCallListening, 200);
        });
      } catch {
    ...
    try { r.start(); } catch { if (callLoopRef.current) setTimeout(startCallListening, 700); }
  };
    ...
    return (
      ...
      {callFullscreen && (
        <div style={{
          ...
          {/* Background radial glow — shifts colour with status */}
          <div style={{
            position:'absolute', top:'20%', left:'50%', transform:'translateX(-50%)',
            width:480, height:480, borderRadius:'50%',
            background:`radial-gradient(circle, ${sc}22 0%, transparent 70%)`,
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{display:'flex', alignItems:'center', gap:8}}>
                <div style={{width:8, height:8, borderRadius:'50%', background:sc, boxShadow:`0 0 10px ${sc}`, animation:'pulse 1.4s ease-in-out infinite'}}/>
            ...
            <div style={{display:'flex', flexDirection:'column', alignItems:'center', flexShrink:0}}>
              ...
              <div style={{position:'relative', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:20}}>
                {[120,160,200].map((sz,ri)=>(
                  <div key={ri} style={{
                    position:'absolute', width:sz, height:sz, borderRadius:'50%',
                    border:`1.5px solid ${sc}`,
                    opacity: isActive ? [0.5,0.3,0.15][ri] : 0.07,
                    animation: isActive ? `ping ${1.1+ri*0.35}s cubic-bezier(0,0,0.2,1) infinite` : 'none',
                ...
                <div style={{
                  ...
                  <img src={avatarSrc} alt="Tessa" style={{width:'100%',height:'100%',objectFit:'cover'}}
                    onError={e=>{(e.currentTarget as HTMLImageElement).src='/avatars/cosmic.png';}}/>
                  {callStatus==='speaking' && (
                    <div style={{position:'absolute',inset:0,background:`${sc}18`,animation:'pulse 0.75s ease-in-out infinite'}}/>
                  )}
                </div>
              </div>
            ...
            {/* Waveform — GPU layer per bar via will-change:transform */}
            <div style={{display:'flex',alignItems:'flex-end',justifyContent:'center',gap:3,height:72,width:'100%',padding:'0 28px',flexShrink:0}}>
              {barPattern.map((bh,i)=>(
                <div key={i} className="wave-bar" style={{
                  flex:1, maxWidth:7, borderRadius:'4px 4px 2px 2px',
                  background: isActive ? sc : 'rgba(255,255,255,0.09)',
                  height: isActive ? Math.max(6,bh) : 5,
                  transformOrigin:'bottom center',
                  animation: isActive ? `soundBar ${0.3+Math.sin(i*0.9)*0.2}s ease-in-out infinite alternate` : 'none',
                  animationDelay:`${(i*0.04)%0.6}s`,
                  /* only transition background colour, NOT height (causes layout thrash) */
          ...
          <div style={{
            ...
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'4px 16px 18px'}}>
              ...
              <div style={{position:'relative',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:10}}>
                {[72,96].map((sz,ri)=>(
                  <div key={ri} style={{position:'absolute',width:sz,height:sz,borderRadius:'50%',border:`1.5px solid ${sc}`,opacity:isActive?[0.4,0.2][ri]:0.06,animation:isActive?`ping ${1.0+ri*0.35}s cubic-bezier(0,0,0.2,1) infinite`:'none',animationDelay:`${ri*0.2}s`}}/>
                ))}
              ...
              {/* Mini waveform — grows from bottom */}
              <div style={{display:'flex',alignItems:'flex-end',gap:2,height:32,marginBottom:10}}>
                {barPattern.slice(0,24).map((bh,i)=>(
                  <div key={i} style={{
                    width:2.5, borderRadius:'3px 3px 1px 1px',
                    background: isActive ? `linear-gradient(0deg,${sc},${sc}88)` : 'rgba(255,255,255,0.12)',
                    height: isActive ? Math.max(4, Math.round(bh*0.75)) : 4,
                    transformOrigin:'bottom',
                    animation: isActive ? `soundBar ${0.3+Math.sin(i*0.9)*0.2}s ease-in-out infinite alternate` : 'none',
                    animationDelay:`${(i*0.04)%0.6}s`,
                    transition:'height 0.25s, background 0.3s',
                    boxShadow: isActive ? `0 0 5px ${sc}50` : 'none',
                  }}/>
                ))}
              </div>
              ...
              <div className="space-y-2.5 pb-2">
                ...
                {messages.map((msg,idx)=>{
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`msg-row group flex gap-2.5 ${isUser?'justify-end':'justify-start'} ${isGrouped?'mt-0.5':'mt-2.5'} ${isUser?'pl-10':'pr-2'}`}
                    >
                      {/* LEFT: Tessa T icon — beautiful glowing mark */}
                      {!isUser && (
                        <div className="flex-shrink-0 flex flex-col justify-end" style={{width:34}}>
                          {!isGrouped ? (
                            <div className="relative w-[34px] h-[34px] rounded-2xl flex-shrink-0 mb-0.5 flex items-center justify-center"
                              style={{
                                background: t.isLight
                                  ? `linear-gradient(135deg,#fff 0%,${t.glow}18 100%)`
                                  : `linear-gradient(135deg,${t.glow}28 0%,${t.glow}08 100%)`,
                                border:`1.5px solid ${t.glow}50`,
                                boxShadow: animations
                                  ? `0 0 0 3px ${t.glow}0a, 0 2px 12px ${t.glow}35`
                                  : 'none',
                              }}>
                              {/* Glow halo */}
                              {animations && (
                                <div className="absolute inset-0 rounded-2xl opacity-30"
                                  style={{background:`radial-gradient(circle at 50% 40%,${t.glow}60,transparent 70%)`}}/>
                              )}
                              <span style={{
                                position:'relative',
                                background:`linear-gradient(160deg,${t.glow} 0%,${t.glow}cc 100%)`,
                                ...
                                filter: animations ? `drop-shadow(0 0 6px ${t.glow}90)` : 'none',
                              }}>T</span>
                            </div>
                          ) : (
                            <div style={{width:34}}/>
